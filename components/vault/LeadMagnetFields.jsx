...
export default function LeadMagnetFields({ funnelId, onApprove, onRenderApproveButton, onUnapprove, isApproved, refreshTrigger }) {
    ...
    useEffect(() => {
        if (refreshTrigger && funnelId) {
            console.log('[LeadMagnetFields] Refresh triggered by parent');
            fetchFields(true);
        }
    ...
    useEffect(() => {
        if (previousApprovalRef.current === true && sectionApproved === false && onUnapprove) {
            console.log('[LeadMagnetFields] Section unapproved, notifying parent');
    ...
    const handleFeedbackSave = async (saveData) => {
        ...
        try {
            ...
            // Schedule check for dependency updates
            scheduleCheck(2500);
        } catch (error) {
