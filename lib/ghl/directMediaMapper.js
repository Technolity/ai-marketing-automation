...
export function mapMediaToGHLValues(mediaContent) {
    ...
    if (mediaContent.logo || mediaContent.logoUrl || mediaContent.logo_url) {
        ...
        result['logo_image'] = logoUrl; // Universal logo for all pages
        console.log('[MediaMapper] ✓ Logo mapped:', logoUrl);
    }

    // Optin page mockup/hero image
    if (mediaContent.banner_image || mediaContent.mockup || mediaContent.mockupImage || mediaContent.optin_mockup || mediaContent.product_mockup) {
        // `banner_image` is the LIVE Free Gift / opt-in image field_id; legacy aliases follow.
        const mockupUrl = mediaContent.banner_image || mediaContent.mockup || mediaContent.mockupImage || mediaContent.optin_mockup || mediaContent.product_mockup;
        result['03_optin_mockup_image'] = mockupUrl;
        console.log('[MediaMapper] ✓ Optin mockup image:', mockupUrl);
    }

    // VSL video
    if (mediaContent.vsl_video || mediaContent.vslVideo || mediaContent.mainVideo || mediaContent.main_vsl) {
        // `vsl_video` is the LIVE Appointment Booking video field_id; legacy aliases follow.
        const vslVideoUrl = mediaContent.vsl_video || mediaContent.vslVideo || mediaContent.mainVideo || mediaContent.main_vsl;
        result['03_vsl_video_link'] = toEmbedUrl(vslVideoUrl);
        console.log('[MediaMapper] ✓ VSL video:', result['03_vsl_video_link']);
    }

    // Thank You page video
    if (mediaContent.thankyou_video || mediaContent.thankYouVideo || mediaContent.thank_you_video || mediaContent.confirmationVideo) {
        // `thankyou_video` is the LIVE Thank You video field_id — was missing here, so this
        // path never mapped it ("thank you video disappeared"). Live first, aliases follow.
        const tyVideoUrl = mediaContent.thankyou_video || mediaContent.thankYouVideo || mediaContent.thank_you_video || mediaContent.confirmationVideo;
        result['03_thankyou_page_video_link'] = toEmbedUrl(tyVideoUrl);
        console.log('[MediaMapper] ✓ Thank You video:', result['03_thankyou_page_video_link']);
    }
    ...
    if (mediaContent.bioPhoto || mediaContent.bio_photo || mediaContent.profilePhoto || mediaContent.bio_author) {
        ...
        result['03_vsl_bio_image'] = bioPhotoUrl;
        console.log('[MediaMapper] ✓ Bio photo:', bioPhotoUrl);
    }

    // Testimonial profile pictures (4 pics)
    const testimonialPics = [
        { field: 'testimonial1Photo', altField: 'testimonial_1_photo', ghlKey: '03_vsl_testimonial_review_1_image' },
        { field: 'testimonial2Photo', altField: 'testimonial_2_photo', ghlKey: '03_vsl_testimonial_review_2_image' },
        { field: 'testimonial3Photo', altField: 'testimonial_3_photo', ghlKey: '03_vsl_testimonial_review_3_image' },
        { field: 'testimonial4Photo', altField: 'testimonial_4_photo', ghlKey: '03_vsl_testimonial_review_4_image' }
    ];
    ...
    if (testimonialsMapped === 0 && mediaContent.testimonialPhotos && Array.isArray(mediaContent.testimonialPhotos)) {
        mediaContent.testimonialPhotos.slice(0, 4).forEach((photoUrl, index) => {
            if (photoUrl) {
                result[`03_vsl_testimonial_review_${index + 1}_image`] = photoUrl;
                testimonialsMapped++;
...
const directMediaMapper = {
    mapMediaToGHLValues,
    validateMediaContent
};
export default directMediaMapper;