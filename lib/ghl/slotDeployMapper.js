...
export function buildSlotDeployCustomValues({
    ...
    // LIVE vault field_id FIRST in each chain; legacy aliases are fallback only.
    // `profile_photo` = bio image, `banner_image` = Free Gift/opt-in image, `vsl_video`
    // = appointment booking video. Reading the live field first stops a stale legacy row
    // (bio_author / product_mockup / main_vsl) from shadowing the user's current media.
    ...
    addValue(values, prefixedKey('vsl_video_link'), toEmbedUrl(combinedMedia.vsl_video || combinedMedia.main_vsl || combinedMedia.vslVideo || combinedMedia.mainVideo));
