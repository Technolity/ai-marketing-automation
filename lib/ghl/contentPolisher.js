...
const STANDARD_REPLACEMENTS = [
    ...
    // Schedule Link
    { pattern: /\[Schedule Link\]/gi, replacement: '{{custom_values.schedule_link}}' },
    { pattern: /\[Booking Link\]/gi, replacement: '{{custom_values.schedule_link}}' },
    { pattern: /\[Calendar Link\]/gi, replacement: '{{custom_values.schedule_link}}' },
];
...
const APPOINTMENT_REPLACEMENTS = [
    ...
    // Schedule Link for appointment reminders (uses appointment.meeting_location for GMeet)
    { pattern: /\[Schedule Link\]/gi, replacement: '{{custom_values.schedule_link}}' },
    { pattern: /\[Booking Link\]/gi, replacement: '{{custom_values.schedule_link}}' },
    { pattern: /\[Calendar Link\]/gi, replacement: '{{custom_values.schedule_link}}' },
];
...
/**
 * Replace placeholder text with GHL custom values
 * @param {string} text - Text containing placeholders
 * @param {string} type - 'standard' for emails/SMS, 'appointment' for appointment reminders
 * @returns {string} Text with GHL custom values
 */
export function replaceCustomValues(text, type = 'standard') {
...
export async function polishTextContent(text, fieldType = 'paragraph') {
    ...
    if (!needsAIPolishing(cleaned, fieldType)) {
        return replaceCustomValues(formatForFieldType(cleaned, fieldType), 'standard');
    }
...
- PRESERVE all GHL custom values like {{contact.first_name}} and {{custom_values.schedule_link}}
...
- Return ONLY the cleaned text, nothing else`
        ...
        return replaceCustomValues(response.choices[0].message.content.trim(), 'standard');
    } catch (error) {
