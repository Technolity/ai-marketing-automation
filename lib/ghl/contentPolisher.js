...
// =============================================================================
// GHL CUSTOM VALUE REPLACEMENTS
// =============================================================================

/**
 * Standard placeholder replacements for Emails and SMS
 * Maps common placeholder text to GHL custom values
 */
const STANDARD_REPLACEMENTS = [
    // Contact Information
    { pattern: /\[Contact Full Name\]/gi, replacement: '{{contact.name}}' },
    { pattern: /\[Contact Name\]/gi, replacement: '{{contact.name}}' },
    { pattern: /\[Full Name\]/gi, replacement: '{{contact.name}}' },
    { pattern: /\[Contact First Name\]/gi, replacement: '{{contact.first_name}}' },
    { pattern: /\[First Name\]/gi, replacement: '{{contact.first_name}}' },
    { pattern: /\[Contact Email\]/gi, replacement: '{{contact.email}}' },
    { pattern: /\[Email\]/gi, replacement: '{{contact.email}}' },
    // Schedule link patterns are handled dynamically in replaceCustomValues
];

/**
 * Appointment-specific placeholder replacements
 * Includes standard contact info plus appointment-specific values
 */
const APPOINTMENT_REPLACEMENTS = [
    // Contact Information (same as standard)
    { pattern: /\[Contact Full Name\]/gi, replacement: '{{contact.name}}' },
    { pattern: /\[Contact Name\]/gi, replacement: '{{contact.name}}' },
    { pattern: /\[Full Name\]/gi, replacement: '{{contact.name}}' },
    { pattern: /\[Contact First Name\]/gi, replacement: '{{contact.first_name}}' },
    ...
    { pattern: /\[Appointment Booking Link\]/gi, replacement: '{{appointment.meeting_location}}' },
    { pattern: /\[Meeting Location\]/gi, replacement: '{{appointment.meeting_location}}' },
    { pattern: /\[Meeting Link\]/gi, replacement: '{{appointment.meeting_location}}' },
    { pattern: /\[GMeet Link\]/gi, replacement: '{{appointment.meeting_location}}' },
    { pattern: /\[Zoom Link\]/gi, replacement: '{{appointment.meeting_location}}' },
    { pattern: /\[Call Link\]/gi, replacement: '{{appointment.meeting_location}}' },
    { pattern: /\[Session Link\]/gi, replacement: '{{appointment.meeting_location}}' },
    { pattern: /\[Reschedule Link\]/gi, replacement: '{{appointment.reschedule_link}}' },
    { pattern: /\[Appointment Start Date Time\]/gi, replacement: '{{appointment.start_time}}' },
    { pattern: /\[Appointment Time\]/gi, replacement: '{{appointment.start_time}}' },
    { pattern: /\[Call Time\]/gi, replacement: '{{appointment.start_time}}' },
    { pattern: /\[Start Time\]/gi, replacement: '{{appointment.start_time}}' },
    // Schedule link patterns are handled dynamically in replaceCustomValues
];

/**
 * Replace placeholder text with GHL custom values or a hardcoded booking URL.
 * @param {string} text - Text containing placeholders
 * @param {string} type - 'standard' for emails/SMS, 'appointment' for appointment reminders
 * @param {string|null} scheduleLink - User's actual booking URL; falls back to GHL merge tag
 * @returns {string} Text with placeholders resolved
 */
export function replaceCustomValues(text, type = 'standard', scheduleLink = null) {
    if (!text || typeof text !== 'string') return text || '';

    const replacements = type === 'appointment' ? APPOINTMENT_REPLACEMENTS : STANDARD_REPLACEMENTS;
    ...
    // Apply booking URL replacement — use user's real URL if available, else keep GHL merge tag
    const bookingUrl = scheduleLink || '{{custom_values.schedule_link}}';
    result = result
        .replace(/\[BOOKING_LINK\]/gi, bookingUrl)
        .replace(/\[Schedule Link\]/gi, bookingUrl)
        .replace(/\[Booking Link\]/gi, bookingUrl)
        .replace(/\[Calendar Link\]/gi, bookingUrl)
        .replace(/\{\{custom_values\.schedule_link\}\}/g, bookingUrl);
 ...
 * @param {string|null} scheduleLink - User's actual booking URL; passed to replaceCustomValues
 * @returns {Promise<string>} Polished text
 */
export async function polishTextContent(text, fieldType = 'paragraph', scheduleLink = null) {
    if (!text || typeof text !== 'string') return text || '';
    ...
    if (!needsAIPolishing(cleaned, fieldType)) {
        return replaceCustomValues(formatForFieldType(cleaned, fieldType), 'standard', scheduleLink);
    }
...
Rules:
- PRESERVE ALL HTML TAGS (<p>, <strong>, <ul>, <li>, <ol>, <a>, etc.) - these are REQUIRED for email formatting
- PRESERVE all GHL custom values like {{contact.first_name}} and merge tags
- Remove any JSON formatting or escape sequences (\\n, \\t, \\")
- Keep the content human-readable and professional
- Preserve the meaning exactly - don't add or remove information
- For headlines: Keep it punchy and attention-grabbing (remove any HTML tags from headlines)
- For paragraphs: Ensure proper sentence structure and preserve HTML formatting
- For bullets: Keep concise and impactful
- For emails: Preserve ALL HTML tags and structure - do NOT convert to plain text
...
- Return ONLY the cleaned text, nothing else`
            ...
            temperature: 0.3,
            max_tokens: 2000,  // Increased for HTML-formatted emails (400-600 words + HTML tags)
        });

        return replaceCustomValues(response.choices[0].message.content.trim(), 'standard', scheduleLink);
    } catch (error) {
 ...
 */
export async function polishSMSContent(sms, scheduleLink = null) {
    let polished = await polishTextContent(sms, 'sms', scheduleLink);
