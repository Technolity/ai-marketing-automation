 ...
 */
export function mapToVSLFunnel(sessionData, generatedImages = [], videoUrls = {}) {
  const results = sessionData.results_data || sessionData.generated_content || {};
  const answers = sessionData.answers || {};
  const colors = extractColorScheme(sessionData);
  
  console.log('[VSLMapper] Video URLs provided:', Object.keys(videoUrls).filter(k => videoUrls[k]));
  ...
  const customValues = {
    ...
    optin_headline_text: get(leadMagnet, 'selectedTitle') || 
                         get(leadMagnet, 'titleOptions[0].title') ||
                         get(leadMagnet, 'titleAndHook.mainTitle') || 
                         get(message, 'headlines[0]') || '',
    optin_headline_text_color: colors.primary,
    optin_sub_headline_text: get(leadMagnet, 'titleAndHook.subtitle') || 
                             get(leadMagnet, 'selectedSubtitle') ||
                             get(message, 'outcomePromise.realisticExpectation') || 
                             get(message, 'tagline') || '',
    ...
    optin_popup_headline: get(leadMagnet, 'landingPageCopy.headline') || 
                          'Yes! Send Me the Free Training' ||
                          get(leadMagnet, 'audienceConnection.openingStatement') || '',
    optin_popup_headline_color: colors.primary,

    // ============================================
    // QUESTIONNAIRE PAGE (6 fields)
    // ============================================
    questionnaire_hero_headline: get(message, 'headlines[1]') || 
                                 'Let\'s Find the Perfect Solution for You' ||
                                 get(idealClient, 'desiredOutcomes.transformationStatement') || '',
    ...
    // ============================================
    vsl_hero_headline: get(vsl, 'hookOptions[0]') || 
                       get(message, 'coreMessage') ||
                       get(message, 'headlines[2]') ||
    ...
    vsl_hero_sub_headline: get(message, 'outcomePromise.realisticExpectation') || 
                           get(idealClient, 'desiredOutcomes.primary') ||
                           get(program, 'overview.primaryOutcome') ||
                           get(message, 'tagline') || '',
    ...
    vsl_hero_cta_question_headline: get(message, 'headlines[3]') || 
                                     'Are You Ready to See Real Results?' ||
                                     get(message, 'ctaFraming.positioning') || '',
    ...
    vsl_process_sub_headline: get(program, 'uniqueSolution.solutionName') || 
                              get(program, 'overview.tagline') || 
                              get(program, 'overview.primaryOutcome') ||
                              program.programName || '',
    vsl_process_sub_headline_color: colors.secondary,
    vsl_process_description: '#a1a1aa',
    vsl_process_description_pt_1: get(vsl, 'stepsToSuccess[0].description') || 
                                   get(program, 'stepsToSuccess[0].description') ||
                                   get(vsl, 'stepsToSuccess[0].benefit') ||
                                   get(program, 'threeTips[0].actionStep') || '',
    vsl_process_description_pt_2: get(vsl, 'stepsToSuccess[1].description') || 
                                   get(program, 'stepsToSuccess[1].description') ||
                                   get(vsl, 'stepsToSuccess[1].benefit') ||
                                   get(program, 'threeTips[1].actionStep') || '',
    vsl_process_description_pt_3: get(vsl, 'stepsToSuccess[2].description') || 
                                   get(program, 'stepsToSuccess[2].description') ||
                                   get(vsl, 'stepsToSuccess[2].benefit') ||
                                   get(program, 'threeTips[2].actionStep') || '',
    vsl_process_description_pt_4: get(vsl, 'stepsToSuccess[3].description') || 
                                   get(program, 'stepsToSuccess[3].description') ||
                                   get(vsl, 'stepsToSuccess[3].benefit') ||
                                   get(vsl, 'threeTips[0].whyItWorks') || '',
    vsl_process_description_pt_5: get(vsl, 'threeTips[0].actionStep') || 
                                   get(program, 'threeTips[0].whyItMatters') ||
                                   get(vsl, 'threeTips[0].tipContent') || '',
    ...
    vsl_testimonial_sub_headline: get(vsl, 'socialProofMentions[0]') || 
                                   get(program, 'deliverables.testimonial') ||
                                   'See how others have transformed their results' || '',
    ...
    vsl_bio_description: get(story, 'shortVersion') || 
                         get(story, 'storyArc.resolution') ||
                         get(story, 'keyThemes[0]') ||
                         get(story, 'fullStoryScript')?.substring(0, 300) || '',
    vsl_bio_description_colour: '#a1a1aa',

    // ============================================
    // VSL PAGE - CTA SECTION (5 fields)
    // ============================================
    vsl_cta_headline: get(vsl, 'closingSequence.finalCTA') || 
                      vsl.callToActionName || 
                      'Ready to Get Started? Book Your Call Now',
    vsl_cta_headline_color: '#ffffff',
    vsl_cta_sub_headline: get(message, 'ctaFraming.emotionalState') || 
                          get(program, 'callToActionName') ||
    ...
    // ============================================
    booking_calender_headline: get(program, 'callToActionName') || 
    ...
    // ============================================
    // VIDEO URLs (3 fields) - NEW!
    // ============================================
    vsl_video_url: toEmbedUrl(videoUrls.main_vsl || ''),
    testimonial_video_url: toEmbedUrl(videoUrls.testimonial_video || ''),
    thankyou_video_url: toEmbedUrl(videoUrls.thankyou_video || '')
  };

  // Convert null/undefined to empty strings (keep all 88 fields)
  // GHL requires all custom values to exist, even if empty
  ...
  console.log(`[VSLMapper] Mapped ${Object.keys(customValues).length} custom values`);
...
export function validateVSLMapping(customValues) {
  ...
  return {
    ...
    totalFields: present,
    expectedFields: 91, // Updated: 88 + 3 video URLs
    coverage: Math.round((present / 91) * 100),
    missing,
    status: missing.length === 0 ? 'PASS' : 'INCOMPLETE'
  };
}

const vslFunnelMapper = {
  mapToVSLFunnel,
  validateVSLMapping,
  extractColorScheme
};
export default vslFunnelMapper;