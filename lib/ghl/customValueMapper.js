...
/**
 * Validate session data has required content for GHL mapping
 */
export function validateSessionData(sessionData) {
  const required = ['business_name', 'answers', 'generated_content'];
  const missing = required.filter(field => !sessionData || !sessionData[field]);

  // Check for minimum generated content
  const generatedContent = sessionData?.generated_content || {};
  const contentKeys = Object.keys(generatedContent);

  if (contentKeys.length < 3) {
    missing.push('minimum_generated_content');
  }
...
export default customValueMapper;