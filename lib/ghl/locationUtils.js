...
export async function findLocationByEmail(email, accessToken, companyId, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      // ── Strategy 1: search locations by email field ──────────────────────
      ...
      if (!locRes.ok) {
        console.warn(`[locationUtils] Location search HTTP ${locRes.status}`);
        if (locRes.status === 401 || locRes.status === 403) break;
      } else {
        const data = await locRes.json();
        const locations = data.locations || [];
        console.log(`[locationUtils] Location search returned ${locations.length} results`);
        ...
        if (match) {
          console.log(`[locationUtils] Found location ${match.id} via location email (attempt ${attempt})`);
          return { locationId: match.id, locationName: match.name || null };
        }
      }

      // ── Strategy 2: search GHL users by email, then get their locationId ─
      // GHL SaaS Configurator creates a user with the payment email even when
      // the location itself has a different business email.
      console.log(`[locationUtils] Falling back to user search for ${email}`);
      const usersRes = await fetch(
        `https://services.leadconnectorhq.com/users/search?companyId=${companyId}&query=${encodeURIComponent(email)}&limit=20`,
        { headers: GHL_HEADERS }
      );

      if (!usersRes.ok) {
        console.warn(`[locationUtils] User search HTTP ${usersRes.status}`);
      } else {
        const usersData = await usersRes.json();
        const users = usersData.users || [];
        console.log(`[locationUtils] User search returned ${users.length} results`);
        ...
        if (userMatch) {
          // Extract locationId from user roles — GHL stores it under roles.locationIds[]
          const locationId =
            userMatch.roles?.locationId ||
            userMatch.roles?.locationIds?.[0] ||
            userMatch.locationIds?.[0] ||
            (Array.isArray(userMatch.roles) ? userMatch.roles[0]?.locationId : null);

          if (locationId) {
            // Fetch location name
            let locationName = null;
            try {
              const locDetailRes = await fetch(
                `https://services.leadconnectorhq.com/locations/${locationId}`,
                { headers: GHL_HEADERS }
              );
              if (locDetailRes.ok) {
                const locDetail = await locDetailRes.json();
                locationName = locDetail.location?.name || locDetail.name || null;
              }
            } catch (e) {
              console.warn('[locationUtils] Could not fetch location name:', e.message);
            }

            console.log(`[locationUtils] Found location ${locationId} via user email match (attempt ${attempt})`);
            return { locationId, locationName };
          } else {
            console.warn(`[locationUtils] User matched but no locationId found. roles:`, JSON.stringify(userMatch.roles));
          }
        }
      }
