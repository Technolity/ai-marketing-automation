...
export async function pushVaultToGHL({
  ...
  try {
    ...
    // Map vault content to custom values
    const allGeneratedValues = mapVaultToCustomValues(
      mergedContent,
      allFields.filter(f => f.section_id === 'media') // Pass media fields separately if mapVaultToCustomValues expects them specifically
    );
    const totalValues = Object.keys(allGeneratedValues).length;
