...
import { mapBrandColorsToGHLValues, validateBrandColors } from './directBrandColorsMapper.js';
import { mapMediaToGHLValues, validateMediaContent } from './directMediaMapper.js';
import { mapCompanyInfoToGHLValues, validateCompanyInfo } from './directCompanyInfoMapper.js';
import { FUNNEL_COPY_MAP } from './customValuesMap.js';  // Import correct 03_* mappings
import crypto from 'crypto';

/**
 * Parse brand colors from string format to object format
 * Handles various input formats:
 * - "Primary: #00BFFF, Secondary: #1A1A1D"
 * - "#00BFFF, #1A1A1D, #FFD700"
 * - "Blue (#00BFFF), Gray (#1A1A1D)"
 */
function parseBrandColorsString(colorString) {
  if (!colorString || typeof colorString !== 'string') {
    return null;
  }

  const result = {};
  const hexPattern = /#[0-9A-Fa-f]{3,8}/gi;

  // Extract all hex colors
  const foundColors = colorString.match(hexPattern) || [];

  // Try to find labeled colors first
  const labelPatterns = [
    { key: 'primary', patterns: ['primary', 'main', 'brand'] },
    { key: 'secondary', patterns: ['secondary', 'accent', 'highlight'] },
    { key: 'accent', patterns: ['accent', 'tertiary', 'third'] },
    { key: 'background', patterns: ['background', 'bg', 'back'] },
    { key: 'text', patterns: ['text', 'body', 'content'] },
    { key: 'heading', patterns: ['heading', 'headline', 'header', 'title'] },
    { key: 'cta', patterns: ['cta', 'button', 'action'] }
  ];

  const lowerString = colorString.toLowerCase();

  for (const { key, patterns } of labelPatterns) {
    for (const pattern of patterns) {
      // Try "pattern: #hex" or "pattern (#hex)" or "pattern - #hex"
      const searchPatterns = [
        new RegExp(`${pattern}[:\\s-]*?(#[0-9a-f]{3,8})`, 'i'),
        new RegExp(`${pattern}[^#]*?(#[0-9a-f]{3,8})`, 'i')
      ];

      for (const regex of searchPatterns) {
        const match = colorString.match(regex);
        if (match && match[1]) {
          result[key] = match[1].toUpperCase();
          break;
        }
      }
      if (result[key]) break;
  ...
  // If no labeled colors found, assign by position
  if (Object.keys(result).length === 0 && foundColors.length > 0) {
    const colorKeys = ['primary', 'secondary', 'accent', 'background', 'text', 'cta'];
    foundColors.slice(0, colorKeys.length).forEach((color, idx) => {
      result[colorKeys[idx]] = color.toUpperCase();
    });
  }

  console.log('[parseBrandColorsString] Parsed:', result);
 ...
 *
 * @param {boolean} updateOnly - If true, only updates existing values (no creates)
 */
export async function pushAllContentToGHL({
  ...
  accessToken,
  onProgress,
  updateOnly = true // Default to UPDATE-ONLY mode
}) {
...
export async function pushVaultToGHL({
  ...
  onProgress,
  updateOnly = true, // Default to UPDATE-ONLY mode
  skipAI = false // Skip AI generation for fast deploy - uses direct mappings only
}) {
  const startTime = Date.now();
  ...
  try {
    ...
    // Parallel fetch: GHL values + previous operation + vault content + media + wizard answers
    const [existingValues, previousOpResult, vaultContent, mediaFields, wizardFunnel] = await Promise.all([
      fetchGHLCustomValues(locationId, accessToken),
      supabaseAdmin
        .from('ghl_push_operations')
        .select('*')
    ...
    // Extract wizard answers (intake form data)
    const wizardAnswers = wizardFunnel?.data?.wizard_answers || {};
    console.log(`[VaultPush] Found wizard answers with keys:`, Object.keys(wizardAnswers));
    ...
    // Build existing values map with comprehensive normalization
    const existingMap = new Map();
    existingValues.forEach(v => {
      // Original name
      existingMap.set(v.name, v);
      ...
      // Normalize spaces to underscores: "02 VSL Text" → "02_vsl_text"
      const underscoreVersion = v.name.toLowerCase().replace(/\s+/g, '_');
      existingMap.set(underscoreVersion, v);

      // Normalize underscores to spaces: "02_vsl_text" → "02 vsl text"
      const spaceVersion = v.name.toLowerCase().replace(/_/g, ' ');
      existingMap.set(spaceVersion, v);

      // Also add original with opposite normalization
      if (v.name.includes('_')) {
        existingMap.set(v.name.replace(/_/g, ' '), v);
      }
      if (v.name.includes(' ')) {
        existingMap.set(v.name.replace(/\s+/g, '_'), v);
      }
    });
    ...
    try {
      ...
      let brandColorsRaw = null;

      // Try intake_form section first
      if (mergedContentObj.intake_form?.brandColors) {
        brandColorsRaw = mergedContentObj.intake_form.brandColors;
      ...
      // Try media section (might have brandColors field)
        ...
        brandColorsRaw = fieldsBySection.media.brandColors;
        console.log('[VaultPush] Found brandColors in media fields');
      }
      // Try root level
      else if (mergedContentObj.brandColors) {
        brandColorsRaw = mergedContentObj.brandColors;
        console.log('[VaultPush] Found brandColors at root level');
      }
      // Try wizard answers (Q15 brand colors from intake form)
      else if (wizardAnswers.brandColors || wizardAnswers['15']) {
        brandColorsRaw = wizardAnswers.brandColors || wizardAnswers['15'];
        console.log('[VaultPush] Found brandColors in wizard answers');
      }

      // Parse brand colors - handle both string and object formats
      if (brandColorsRaw) {
        if (typeof brandColorsRaw === 'object') {
          // Already an object, use directly
          brandColorsInput = brandColorsRaw;
          console.log('[VaultPush] brandColors is already an object');
        } else if (typeof brandColorsRaw === 'string' && brandColorsRaw.trim()) {
          // Parse string format like "Primary: #00BFFF, Secondary: #1A1A1D"
          console.log('[VaultPush] Parsing brandColors string:', brandColorsRaw.substring(0, 100));
          brandColorsInput = parseBrandColorsString(brandColorsRaw);
          console.log('[VaultPush] Parsed brandColors:', brandColorsInput);
        }
      }
      ...
      // Step 1: Execute prompt-based generation (skip if skipAI=true for fast deploy)
      let promptedValues = {};

      if (skipAI) {
        console.log('[VaultPush] SKIP AI mode - bypassing AI generation for fast deploy');
        onProgress?.({ step: 'skip_ai', progress: 60, message: 'Fast deploy mode - skipping AI generation...' });
      } else {
        // Normal AI-powered generation
        promptedValues = await generateCustomValuesWithPrompts(
          mergedContentObj,
          (promptProgress) => {
      ...
      onProgress?.({ step: 'brand_colors_mapping', progress: 68, message: 'Extracting brand colors...' });

      // Step 2.8: BRAND COLORS DIRECT MAPPING
      // Extract brand colors from intake form (reuse brandColorsInput from line 630)
      console.log('[VaultPush] Extracting brand colors for direct mapping...');
      const brandColorValues = mapBrandColorsToGHLValues(brandColorsInput);

      // Validate brand colors
      const brandColorsValidation = validateBrandColors(brandColorsInput);
      console.log('[VaultPush] Brand colors validation:', {
        ...brandColorsValidation.stats,
        warnings: brandColorsValidation.warnings.length
      });

      console.log(`[VaultPush] Brand colors mapping added ${Object.keys(brandColorValues).length} color values`);

      // Merge brand color values
      const mergedWithBrandColors = {
        ...mergedWithAppointmentReminders,
        ...brandColorValues // Brand color values added
      };

      onProgress?.({ step: 'media_mapping', progress: 69, message: 'Extracting media library assets...' });

      // Step 2.9: MEDIA LIBRARY DIRECT MAPPING
      // Extract media URLs from media library
      console.log('[VaultPush] Extracting media library custom values...');

      // Media is stored in vault_content_fields, not vault_content (no AI generation)
      // So we need to check fieldsBySection.media first
      const mediaContent = mergedContentObj.media || fieldsBySection.media || {};
      console.log('[VaultPush] Media content keys:', Object.keys(mediaContent));

      const mediaValues = mapMediaToGHLValues(mediaContent);

      // Validate media content
      const mediaValidation = validateMediaContent(mediaContent);
      console.log('[VaultPush] Media library validation:', {
        ...mediaValidation.stats,
        warnings: mediaValidation.warnings.length
      });

      console.log(`[VaultPush] Media library mapping added ${Object.keys(mediaValues).length} media values`);

      // Merge media values
      const mergedWithMedia = {
        ...mergedWithBrandColors,
        ...mediaValues // Media values added
      };

      onProgress?.({ step: 'company_info_mapping', progress: 69.5, message: 'Extracting company information...' });

      // Step 2.10: COMPANY INFO DIRECT MAPPING
      // Extract company info from intake form (wizard answers)
      console.log('[VaultPush] Extracting company info from intake form...');

      // Company info is stored in wizard_answers from the intake form
      // Try multiple sources: wizardAnswers > fieldsBySection > mergedContentObj.intake_form
      const companyInfoSource = Object.keys(wizardAnswers).length > 0 ? wizardAnswers
        : (mergedContentObj.intake_form || fieldsBySection.intake_form || {});

      console.log('[VaultPush] Company info source keys:', Object.keys(companyInfoSource));
      const companyInfoValues = mapCompanyInfoToGHLValues(companyInfoSource);

      // Validate company info
      const companyInfoValidation = validateCompanyInfo(companyInfoSource);
      console.log('[VaultPush] Company info validation:', {
        ...companyInfoValidation.stats,
        warnings: companyInfoValidation.warnings.length
      });

      console.log(`[VaultPush] Company info mapping added ${Object.keys(companyInfoValues).length} company values`);

      // Merge company info values
      const mergedWithCompanyInfo = {
        ...mergedWithMedia,
        ...companyInfoValues // Company info values added
      };
      ...
      allGeneratedValues = fillMissingWithInference(mergedWithCompanyInfo, ALL_CUSTOM_VALUE_KEYS, mergedContentObj);
    ...
    for (const [key, value] of entries) {
      // Improved key normalization - check multiple variations
      const normalizedKey = key.toLowerCase().replace(/\s+/g, '_');
      ...
      // If UPDATE-ONLY mode and no existing value, skip creation
      if (updateOnly && !existing) {
        console.log(`[VaultPush] SKIPPED: ${key} (updateOnly mode, no existing value)`);
        results.skipped++;
        processedCount++;
        continue;
      }
      ...
      // Push to GHL
      let result = await pushSingleCustomValue(
        locationId,
        accessToken,
        key,
        value,
      ...
      // If POST failed with "already exists", try to find existing and retry with PUT
      if (!result.success && result.error?.includes('already exists')) {
        console.log(`[VaultPush] Duplicate detected, searching for existing value: ${key}`);

        // Fetch current values again to find the duplicate
        const currentValues = await fetchGHLCustomValues(locationId, accessToken);
        const duplicate = currentValues.find(v =>
          v.name === key ||
          v.name.toLowerCase() === key.toLowerCase() ||
          v.name.toLowerCase().replace(/\s+/g, '_') === normalizedKey ||
          v.name.toLowerCase().replace(/_/g, ' ') === spaceKey
        );

        if (duplicate) {
          console.log(`[VaultPush] Found existing value with ID ${duplicate.id}, retrying with PUT`);
          result = await pushSingleCustomValue(
            locationId,
            accessToken,
            key,
            value,
          ...
          existing = duplicate; // Mark as existing for correct counting
        }
      }
 ...
 * Maps the 4-page structure (optinPage, salesPage, calendarPage, thankYouPage) to GHL custom values
 */
export function extractFunnelCopyCustomValues(funnelCopyContent) {
  const customValues = {};
  ...
  // Use the centralized mapping from customValuesMap.js (03_* keys)
  const FUNNEL_COPY_FIELD_MAPPINGS = FUNNEL_COPY_MAP;
  ...
  // Extract from calendarPage
  if (funnelCopyContent.calendarPage || funnelCopyContent.bookingPage) {
    const calendarData = funnelCopyContent.calendarPage || funnelCopyContent.bookingPage;
    Object.entries(calendarData).forEach(([key, value]) => {
      const ghlKey = FUNNEL_COPY_FIELD_MAPPINGS.calendarPage[key];
      if (ghlKey && value) {
        customValues[ghlKey] = value;
      }
    });
...
/**
 * Push specific vault sections to GHL for a user (admin-initiated, bypasses user auth).
 * Uses direct mappers — no AI generation. Fast and targeted.
 *
 * @param {object} opts
 * @param {string} opts.userId        - Funnel owner's user ID
 * @param {string} opts.funnelId      - Funnel ID
 * @param {string} opts.locationId    - GHL location ID
 * @param {string} opts.accessToken   - Valid GHL access token for the location
 * @param {string[]} opts.sectionIds  - Section IDs to push (e.g. ['emails','sms'])
 * @param {Function} [opts.onProgress]
 */
export async function pushSectionsToGHL({
  userId,
  funnelId,
  locationId,
  accessToken,
  ...
  existingValues.forEach(v => {
    ...
    existingMap.set(v.name.toLowerCase().replace(/_/g, ' '), v);
    if (v.name.includes('_')) existingMap.set(v.name.replace(/_/g, ' '), v);
    if (v.name.includes(' ')) existingMap.set(v.name.replace(/\s+/g, '_'), v);
  });

  onProgress?.({ step: 'fetch_content', progress: 10, message: 'Loading vault sections…' });

  const [vaultRes, fieldsRes, wizardRes] = await Promise.all([
    supabaseAdmin
      .from('vault_content')
      .select('section_id, content')
      .eq('funnel_id', funnelId)
  ...
  const wizardAnswers = wizardRes.data?.wizard_answers || {};

  // Build fieldsBySection for all fields (needed for media + overrides)
  const fieldsBySection = {};
  (fieldsRes.data || []).forEach(field => {
    if (!fieldsBySection[field.section_id]) fieldsBySection[field.section_id] = {};
    let val = field.field_value;
    if ((field.field_type === 'array' || field.field_type === 'object') && typeof val === 'string') {
      try { if (val.startsWith('[') || val.startsWith('{')) val = JSON.parse(val); } catch (_) {}
    }
    fieldsBySection[field.section_id][field.field_id] = val;
  });

  // Merge vault_content blobs with field overrides, keyed by section_id
  const mergedContent = {};
  (vaultRes.data || []).forEach(section => {
    const overrides = fieldsBySection[section.section_id] || {};
    mergedContent[section.section_id] = { ...(section.content || {}), ...overrides };
  });
  // Sections that exist only in fields (e.g. media)
  Object.keys(fieldsBySection).forEach(sid => {
    if (!mergedContent[sid]) mergedContent[sid] = fieldsBySection[sid];
  });

  onProgress?.({ step: 'map_content', progress: 30, message: 'Mapping section values…' });

  let sectionValues = {};

  for (const sectionId of sectionIds) {
    const content = mergedContent[sectionId];

    switch (sectionId) {
      case 'emails':
        Object.assign(sectionValues, mapEmailsToGHLValues(content));
        break;

      case 'sms':
        Object.assign(sectionValues, mapSMSToGHLValues(content));
        break;

      case 'appointmentReminders':
        Object.assign(sectionValues, mapAppointmentRemindersToGHLValues(content));
        break;

      case 'funnelCopy':
        Object.assign(sectionValues, extractFunnelCopyCustomValues(content));
        break;

      case 'media': {
        const mediaContent = content || fieldsBySection['media'] || {};
        Object.assign(sectionValues, mapMediaToGHLValues(mediaContent));
        break;
      }

      case 'intake_form': {
        // Brand colors + company info come from the intake form / wizard answers
        const source = Object.keys(wizardAnswers).length > 0 ? wizardAnswers : (content || {});
        let brandColorsRaw = source.brandColors || wizardAnswers.brandColors || null;
        let brandColorsInput = null;
        if (brandColorsRaw) {
          brandColorsInput = typeof brandColorsRaw === 'object'
            ? brandColorsRaw
            : parseBrandColorsString(brandColorsRaw);
        }
        Object.assign(sectionValues, mapBrandColorsToGHLValues(brandColorsInput));
        Object.assign(sectionValues, mapCompanyInfoToGHLValues(source));
        break;
      }

      default: {
        // All other sections (idealClient, message, story, offer, vsl, bio, salesScripts, etc.)
        // use the legacy mapper filtered to just this section
        if (content) {
          const legacyValues = mapVaultToCustomValues(
            [{ section_id: sectionId, content }],
            sectionId === 'media' ? (fieldsRes.data || []).filter(f => f.section_id === 'media') : []
          );
          Object.assign(sectionValues, legacyValues);
        }
        break;
      }
    }
  ...
  const totalValues = Object.keys(sectionValues).length;

  if (totalValues === 0) {
    return {
      success: true,
      summary: { total: 0, created: 0, updated: 0, failed: 0, skipped: 0, duration: '0s' },
      details: { created: [], updated: [], failed: [] },
    };
  }
  ...
  const results = { created: [], updated: [], failed: [], skipped: 0 };
  let processedCount = 0;
  ...
  for (const [key, value] of Object.entries(sectionValues)) {
    ...
    const result = await pushSingleCustomValue(locationId, accessToken, key, value, existing?.id || null);

    if (result.success) {
      if (existing) {
        results.updated.push({ key, value: String(value).substring(0, 100) });
      } else {
        results.created.push({ key, value: String(value).substring(0, 100) });
      }
    } else {
      results.failed.push({ key, error: result.error });
    }
    ...
    onProgress?.({
      step: 'push_values',
      progress: 40 + Math.round((processedCount / totalValues) * 55),
      message: `Pushed ${processedCount}/${totalValues} values…`,
    });

    await new Promise(r => setTimeout(r, 120));
  }

  onProgress?.({ step: 'complete', progress: 100, message: 'Section push complete!' });
