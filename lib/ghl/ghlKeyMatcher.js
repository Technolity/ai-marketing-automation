 ...
 * Handles special cases like "Subheadline" -> "Sub-Headline", "Optin" -> "Opt In"
 */
...
/**
 * Find existing GHL custom value ID using 11-level fallback matching
 *
 * @param {Map} existingMap - Map from buildExistingMap()
 * @param {string} ghlKey - The GHL key to find (e.g., "03_vsl_bio_image")
 * @returns {string|null} The existing value ID or null if not found
 */
export function findExistingId(existingMap, ghlKey) {
    // 1. Exact match
    if (existingMap.has(ghlKey)) return existingMap.get(ghlKey).id;

    // 2. Lowercase
    const lower = ghlKey.toLowerCase();
    if (existingMap.has(lower)) return existingMap.get(lower).id;

    // 3. Replace spaces with underscores
    const spacesToUnder = ghlKey.replace(/\s+/g, '_');
    if (existingMap.has(spacesToUnder)) return existingMap.get(spacesToUnder).id;

    // 4. Lowercase + replace spaces
    const lowerUnder = lower.replace(/\s+/g, '_');
    if (existingMap.has(lowerUnder)) return existingMap.get(lowerUnder).id;

    // 5. Replace underscores with spaces (GHL Title Case format)
    const underToSpaces = ghlKey.replace(/_/g, ' ');
    if (existingMap.has(underToSpaces)) return existingMap.get(underToSpaces).id;

    // 6. Title Case with spaces: "03_vsl_bio_image" → "03 VSL Bio Image"
    const titleCase = ghlKey
        .replace(/_/g, ' ')
        .replace(/\b\w/g, c => c.toUpperCase());
    if (existingMap.has(titleCase)) return existingMap.get(titleCase).id;

    // 7. Lowercase with spaces
    const lowerSpaces = ghlKey.replace(/_/g, ' ').toLowerCase();
    if (existingMap.has(lowerSpaces)) return existingMap.get(lowerSpaces).id;
    ...
    if (existingMap.has('03 ' + titleCaseNoPrefix)) return existingMap.get('03 ' + titleCaseNoPrefix).id;
    if (existingMap.has('02 ' + titleCaseNoPrefix)) return existingMap.get('02 ' + titleCaseNoPrefix).id;

    // 9. Handle GHL's hyphenated naming: "subheadline" → "Sub-Headline"
    const ghlFormat = toGhlFormat(ghlKey);
    if (existingMap.has(ghlFormat)) return existingMap.get(ghlFormat).id;

    // 10. Try with 03/02 prefix in GHL format
    const ghlFormatNoPrefix = toGhlFormat(withoutPrefix);
    if (existingMap.has('03 ' + ghlFormatNoPrefix)) return existingMap.get('03 ' + ghlFormatNoPrefix).id;
    if (existingMap.has('02 ' + ghlFormatNoPrefix)) return existingMap.get('02 ' + ghlFormatNoPrefix).id;

    // 11. Try lowercase hero → "hero" (GHL keeps some words lowercase)
    const ghlFormatLowerHero = ghlFormat.replace(/Hero/g, 'hero');
    if (existingMap.has(ghlFormatLowerHero)) return existingMap.get(ghlFormatLowerHero).id;
