 ...
 *
 * @param {Array} existingValues - Array of {id, name, value} from GHL API
 * @returns {Map} Map with multiple key formats pointing to {id, name, value}
 */
export function buildExistingMap(existingValues) {
    const existingMap = new Map();

    existingValues.forEach(v => {
        const entry = { id: v.id, name: v.name, value: v.value };
...
/**
 * Normalize a value for comparison against GHL's stored value.
 * GHL stores everything as strings and may trim whitespace.
