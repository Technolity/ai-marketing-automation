...
export async function createGHLUserForUser(userId, requestId = 'helper') {
    try {
        ...
        // 2. Get active subaccount (get most recent if multiple exist)
        ...
        fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/email/ghl-welcome`, {
            ...
            if (!response.ok) {
                ...
                console.error(`[${requestId}] [GHL User Helper] Email send failed:`, {
                    status: response.status,
                    error: errorText
                });
            } else {
                console.log(`[${requestId}] [GHL User Helper] Welcome email sent successfully`);
            }
        }).catch(err => {
            console.error(`[${requestId}] [GHL User Helper] Email send error:`, {
                message: err.message,
                stack: err.stack
            });
        });
