 ...
 */
export async function generateAuthUrl(platform, workspaceId, { connectionType } = {}) {
  const body = {
    platform,
  ...
  // Some providers require extra data under `platform_data`, keyed by the
  // provider name (e.g. platform_data.instagram.connection_type) — NOT flat.
  if (platform === 'instagram') {
    // 'instagram' → Login with Instagram (default; needs a Professional account)
    // 'facebook'  → Login with Facebook (connects the IG account linked to a FB Page)
    body.platform_data = { instagram: { connection_type: connectionType || 'instagram' } };
  } else if (platform === 'linkedin') {
    // With Post for Me's provided credentials this must be 'organization'.
    body.platform_data = { linkedin: { connection_type: connectionType || 'organization' } };
  }

  // TEMP DEBUG — remove once Instagram connect is confirmed working.
  console.log('[PFM auth-url] request body:', JSON.stringify(body));
...
/**
 * Get a connected account's feed with engagement metrics.
 * Requires the account to have been connected with the `feeds` permission
 * (generateAuthUrl already requests it). Returns every post on the account —
 * published via this API or natively — each with lifetime metrics when
 * `expand=metrics` is set.
 *
 * Metric availability varies by platform (e.g. Bluesky has no view count,
 * LinkedIn analytics only for Company Pages).
 *
 * @param {string} accountId - PostForMe spc_... account ID
 * @param {Object} [opts]
 * @param {boolean} [opts.metrics=true] - include engagement metrics
 * @param {number}  [opts.limit=50]
 * @returns {Array} feed posts
 */
export async function getAccountFeed(accountId, { metrics = true, limit = 50 } = {}) {
  const params = new URLSearchParams({ limit: String(limit) });
  if (metrics) params.set('expand', 'metrics');
  const data = await pfmFetch(`/v1/social-account-feeds/${accountId}?${params}`);
  return data.data || [];
}
