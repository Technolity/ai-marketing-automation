...
export async function importSnapshotToSubAccount(userId, snapshotId) {
    try {
        ...
        // Import snapshot - Use AGENCY token, not location token
        // The snapshot share/link API requires agency-level authentication
        console.log('[GHL Snapshot] Importing snapshot:', snapshotId);
        console.log('[GHL Snapshot] Using agency token for import (not location token)');
        ...
        const importResponse = await fetch(
            `https://services.leadconnectorhq.com/snapshots/share/link`,
            {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${tokenData.access_token}`, // Agency token
                    'Content-Type': 'application/json',
                    'Version': '2021-07-28',
                },
                body: JSON.stringify({
                    snapshot_id: snapshotId, // Try snake_case
                    locationId: subaccount.location_id,
                    companyId: companyId,
                    type: 'location', // May be required
                }),
            }
        );
