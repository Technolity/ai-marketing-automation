...
export async function importSnapshotToSubAccount(userId, snapshotId) {
    try {
        ...
        // Try with companyId in query params AND body AND header
        ...
        const importResponse = await fetch(
            importUrl,
            {
                ...
                headers: {
                    ...
                    'Content-Type': 'application/json',
                    'Version': '2021-07-28',
                    'X-Company-Id': companyId, // Try as header too
                },
                body: JSON.stringify({
                    snapshotId: snapshotId,
                    snapshot_id: snapshotId, // Try both camelCase and snake_case
                    locationId: subaccount.location_id,
                    companyId: companyId,
                    company_id: companyId, // Try both
                    type: 'location',
                }),
            }
        );
