...
export async function importSnapshotToSubAccount(userId, snapshotId) {
    try {
        ...
        const requestBody = {
            snapshot_id: snapshotId, // snake_case
            relationship_number: relationshipNumber,// Required unique identifier
            share_type: 'agency_link',// Share from agency to location
            location_id: subaccount.location_id, // Target location to import into (correct parameter name)
        };
...
export async function createGHLUser(userData) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error('[GHL User] API error:', errorText);
