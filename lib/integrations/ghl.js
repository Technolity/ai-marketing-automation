...
export async function updateCustomValues(userId, customValues) {
    try {
        ...
        // Also add delay to avoid rate limiting (429 errors)
        const results = [];
        let firstError = null;
        let successCount = 0;
            ...
            try {
                const updateResponse = await fetch(
                    `https://services.leadconnectorhq.com/locations/${subaccount.location_id}/customValues`,
                    {
                        ...
                        // GHL API expects { name, value } format, not { customValues: [...] }
                        body: JSON.stringify({
                            name: key,
                            value: value
                        }),
                ...
                if (updateResponse.ok) {
                    ...
                    // Don't log every error, just log first few and rate limit errors
                    if (updateResponse.status === 429) {
                        console.error(`[GHL CustomValues] Rate limited at ${key}, waiting...`);
                        await new Promise(r => setTimeout(r, 1000)); // Wait 1 second on rate limit
                    } else if (results.length < 5) {
                        console.error(`[GHL CustomValues] Failed to update ${key}:`, updateResponse.status, errorBody);
                    }
                    if (!firstError) {
                        firstError = errorBody.message || `Status ${updateResponse.status}`;
                    }
                    results.push({ key, success: false, error: errorBody.message });
                }
                ...
                console.error(`[GHL CustomValues] Error updating ${key}:`, err.message);
