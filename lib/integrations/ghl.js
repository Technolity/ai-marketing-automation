    ...
    try {
        ...
        try {
            ...
            if (listResponse.ok) {
                ...
                // Build a map of name -> id for existing custom values
                (listData.customValues || listData || []).forEach(cv => {
                    if (cv.name && cv.id) {
                        existingValues[cv.name] = cv.id;
                    }
                });
                console.log('[GHL CustomValues] Found', Object.keys(existingValues).length, 'existing custom values');
            }
        } catch (err) {
        ...
        const updateOne = async (key, value) => {
            try {
                const existingId = existingValues[key];
                let url, method;
