...
export function mergeEmailChunks(chunk1, chunk2, chunk3, chunk4) {
    ...
    // Unwrap chunks that AI may have wrapped in emailSequence.
    // AI is inconsistent — sometimes returns { email5: {...} } (flat)
    // and sometimes { emailSequence: { email5: {...} } } (wrapped).
    // The merger adds the wrapper, so chunks must be flat.
    const unwrap = (chunk) => {
        if (!chunk) return {};
        if (chunk.emailSequence && typeof chunk.emailSequence === 'object') {
            console.log('[EmailMerger] Unwrapping chunk that was wrapped in emailSequence');
            return chunk.emailSequence;
        }
        return chunk;
    };
    ...
    console.log('[EmailMerger] Chunk contents:', {
        chunk1: Object.keys(c1).filter(k => k.startsWith('email')),
        chunk2: Object.keys(c2).filter(k => k.startsWith('email')),
        chunk3: Object.keys(c3).filter(k => k.startsWith('email')),
        chunk4: Object.keys(c4).filter(k => k.startsWith('email')),
    });
...
const emailMerger = { mergeEmailChunks, validateMergedEmails };

export default emailMerger;
