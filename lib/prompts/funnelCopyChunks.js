...
function buildGlobalContext(data) {
  ...
  // Extract founder name — prioritise full_name injected from user_profiles
  const founderName = data.fullName || bio.name || bio.founderName || intakeForm.founderName || 'the founder';
...
export function chunk2_salesPart1(data) {
  ...
  return `You are an expert Appointment Booking page copywriter creating the opening sections for ${ctx.businessName}.
...
export function chunk3_salesPart2(data) {
  ...
  return `You are an expert Appointment Booking page copywriter creating audience qualification and bio sections for ${ctx.businessName}.
...
OUTPUT FORMAT (valid JSON only):
{
  "salesPage_part2": {
    ...
    "bio_paragraph_text": "Bio story as ONE single flowing paragraph — STRICT MAX 90 WORDS. MUST be a single continuous block of text: NO line breaks, NO paragraph breaks, NO blank lines, NO '\\n' characters (it is stored in a single-line page-builder custom value, so any line break breaks the layout). First-person voice. Cover in one flow: who they are + their struggle/low moment → breakthrough, then what they now do for clients + key result/credential. No dashes, no bullet points. Conversational and warm. Example: 'I'm the founder of [Business]. I was [struggle] while [context] until [turning point], when I discovered [method]. Within [timeframe] I [personal result], and now I help [audience] use [method] to [outcome].'"
  }
}
...
- bio_paragraph_text: Tell the origin story in FIRST PERSON as ONE single continuous paragraph (STRICT MAX 90 WORDS).
  CRITICAL FORMAT: It MUST be a single block of text — NO line breaks, NO paragraph breaks, NO blank lines, NO "\n" characters. It is stored in a single-line page-builder custom value, so any line break or extra paragraph breaks how it renders. Do NOT split it into two paragraphs.
  Cover in one flow: who they are + their struggle/low moment (${ctx.storyLowMoment}) → the turning point / breakthrough (${ctx.storyBreakthrough}), then what they now do for clients + key credential/proof (${ctx.credentials}).
  Rules: First-person voice ("I"), conversational and warm, no dashes, no bullets, no line breaks.
  Example: "I'm the founder of Love Reset Coaching. I was a therapist helping clients with their relationships while my own love life was a disaster, until I found the turning point and met my husband within 8 months. Now I help others use the Secure Attraction Method to stop chasing inconsistency and build secure, steady relationships."
...
export function chunk4_salesPart3(data) {
  ...
  return `You are an expert Appointment Booking page copywriter creating social proof and closing sections for ${ctx.businessName}.
