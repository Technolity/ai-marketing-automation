 ...
 * NEW 79-Field Structure for 03_* GHL Custom Values
 *
 * Structure:
 * - Chunk 1: Optin Page (4 fields)
 * - Chunk 2: Sales Part 1 - Hero + Process + How It Works (23 fields)
 * - Chunk 3: Sales Part 2 - Audience + Call Expectations + Bio (23 fields)
 * - Chunk 4: Sales Part 3 - Testimonials + FAQ + Final CTA + Video (29 fields)
 *
 * Total: 79 fields (4 optin + 75 sales)
...
/**
 * CHUNK 1: OPTIN PAGE (4 fields)
 * Maps to: 03_optin_* GHL custom values
 * Est. Time: 45 seconds
 */
export function chunk1_optinPage(data) {
  ...
  const brandColors = data.brandColors || data.colorPalette || null;
  const colorContext = brandColors
    ? `Brand Colors: Primary (${brandColors.primary?.hex || brandColors.primary}), Secondary (${brandColors.secondary?.hex || brandColors.secondary}), Tertiary (${brandColors.tertiary?.hex || brandColors.tertiary})`
    : '';

  return `You are an expert conversion copywriter creating an opt-in page for ${businessName}.
...
✅ ALL 4 fields must be filled with complete, specific copy
...
✅ Create urgency and desire to opt in
...
{
  "optinPage": {
    "headline_text": "How Top Agents List 10+ Homes Per Month Without Cold Calling",
    "subheadline_text": "Free 8-step checklist shows you the exact social media system used by million-dollar producers",
    "cta_button_text": "Download Checklist",
    "mockup_image": ""
  }
}

Now generate the opt-in page copy for ${businessName}. Return ONLY valid JSON matching the structure above.`;
}
...
/**
 * CHUNK 4: SALES PAGE PART 3 - Testimonials + FAQ + Final CTA + Video (29 fields)
 * Maps to: 03_vsl_testimonial_*, 03_vsl_faq_*, 03_vsl_final_cta_*, 03_vsl_video_link
 * Est. Time: 60 seconds
 */
...
CRITICAL REQUIREMENTS:
✅ ALL 29 fields must be filled with complete, specific copy
✅ NO empty fields, NO placeholders
...
VIDEO (1 field):
- video_link: Leave as empty string ""
...
Now generate Part 3 (Testimonials + FAQ + Final CTA + Video) for ${businessName}. Return ONLY valid JSON.`;
}
