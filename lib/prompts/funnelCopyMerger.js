...
export function mergeFunnelCopyChunks(chunk1, chunk2, chunk3, chunk4) {
    ...
    console.log('[FunnelCopyMerger] salesPage fields:', salesCount, '(expected: 74)');
    console.log('[FunnelCopyMerger] Total fields:', totalCount, '(expected: 78)');
    ...
    if (salesCount !== 74) {
        console.warn('[FunnelCopyMerger] ⚠️ WARNING: salesPage has', salesCount, 'fields (expected: 74)');
    }
    if (totalCount === 78) {
        console.log('[FunnelCopyMerger] ✅ All 78 fields successfully merged');
    }
...
/**
 * Validate merged funnel copy structure (NEW 78-field structure)
...
export function validateMergedFunnelCopy(merged) {
    ...
    // Expected exact field counts for NEW 78-field structure
    const expectedFieldCounts = {
        optinPage: 4,    // headline_text, subheadline_text, cta_button_text, mockup_image
        salesPage: 74    // All VSL/Sales page fields (video moved to Media section)
    };
    ...
    console.log('[FunnelCopyValidator] Total fields:', fieldCount, '(expected: 78)');
    if (issues.length > 0) {
    ...
    if (valid && fieldCount === 78 && emptyFields.length === 0) {
        console.log('[FunnelCopyValidator] ✅ Perfect! All 78 fields present and populated');
    }
