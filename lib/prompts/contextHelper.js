...
export function buildGlobalContext(data) {
    ...
    let desires = [];
    if (idealClientContext.whatTheyWant?.length > 0) {
        desires = idealClientContext.whatTheyWant;
    } else if (rawIdealClient.idealClientSnapshot?.whatTheyWant?.length > 0) {
        desires = rawIdealClient.idealClientSnapshot.whatTheyWant;
    } else if (idealClientContext.topDesires?.length > 0) {
    ...
    // Payment Triggers (whatMakesThemPay) - NEW SCHEMA
    let paymentTriggers = [];
    if (idealClientContext.whatMakesThemPay?.length > 0) {
        paymentTriggers = idealClientContext.whatMakesThemPay;
    } else if (rawIdealClient.idealClientSnapshot?.whatMakesThemPay?.length > 0) {
        paymentTriggers = rawIdealClient.idealClientSnapshot.whatMakesThemPay;
    }

    // Objections (legacy field - not in new schema but kept for backward compatibility)
    let objections = [];
    ...
    // How To Talk To Them (coffee-talk phrases)
    let howToTalkToThem = [];
    if (idealClientContext.howToTalkToThem?.length > 0) {
        howToTalkToThem = idealClientContext.howToTalkToThem;
    } else if (rawIdealClient.idealClientSnapshot?.howToTalkToThem?.length > 0) {
        howToTalkToThem = rawIdealClient.idealClientSnapshot.howToTalkToThem;
    } else if (idealClientContext.wordsTheyUse) {
        // Legacy: convert string to array
        howToTalkToThem = [idealClientContext.wordsTheyUse];
    } else if (rawIdealClient.wordsTheyUse) {
        howToTalkToThem = [rawIdealClient.wordsTheyUse];
    } else if (rawIdealClient.idealClientSnapshot?.wordsTheyUse) {
        howToTalkToThem = [rawIdealClient.idealClientSnapshot.wordsTheyUse];
    }

    // Keep wordsTheyUse for backward compatibility (join array if needed)
    const wordsTheyUse = Array.isArray(howToTalkToThem) && howToTalkToThem.length > 0
        ? howToTalkToThem.join('; ')
        : '';
