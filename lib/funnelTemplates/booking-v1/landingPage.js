...
/**
 * Landing CTA → calendar redirect = a BAKED relative path (NO merge tag).
 * GHL resolves {{custom_values.x}} single-pass, so a tag nested inside baked HTML never
 * resolves — the redirect MUST be a literal. The path is slot-derived so it matches the
 * GHL step slug per offer: slot 1 → /calendar, slot 2 → /calendar2, slot N → /calendar{N}.
 * Relative (same-domain) so it follows whatever domain the funnel is published on.
 */
export const DEFAULT_CALENDAR_PATH = '/calendar';

/** Relative calendar slug for a given ABFv2 slot index (slot 1 = /calendar). */
export function calendarPathForSlot(slot) {
  const n = Number(slot);
  return !n || n <= 1 ? DEFAULT_CALENDAR_PATH : `${DEFAULT_CALENDAR_PATH}${n}`;
}
...
export function renderLandingSegments(data = {}) {
  ...
  // Shared CTA href: a BAKED relative path (slot-derived). No merge tag — see note on DEFAULT_CALENDAR_PATH.
  const calendarPath = safeText(data.calendarPath || DEFAULT_CALENDAR_PATH);
  const ctaHref = `href="${calendarPath}"`;
  ...
  const header = `<header class="v2-header"><div class="v2-header-in"><span class="v2-brandmark">${logoUrl ? `<img class="v2-logo" src="${logoUrl}" alt="${businessName}" />` : `<span class="v2-logo-txt">${businessName}</span>`}</span><a ${ctaHref} class="v2-btn v2-btn--inv v2-header-cta"><span>${heroCta}</span></a></div></header>`;
  const s1 = `${header}
...
<section class="v2-section v2-dark" id="v2-hero">`;
  ...
  <div class="v2-wrap v2-wrap--fat">
    ...
    <div class="v2-btn-wrap v2-btn-wrap--center v2-reveal v2-delay-5"><a ${ctaHref} class="v2-btn v2-btn--inv v2-btn-pulse"><span>${heroCta}</span></a><span class="v2-btn-note">${heroBelow}</span></div>
  </div>
</section>`;
...
<section class="v2-section v2-off" id="v2-faq">
  ...
  <div class="v2-wrap" style="text-align:center;">
    ...
    <div class="v2-btn-wrap v2-btn-wrap--center v2-reveal v2-delay-4"><a ${ctaHref} class="v2-btn v2-btn--inv v2-btn-pulse"><span>${heroCta}</span></a><span class="v2-btn-note">${t(copy.final_subtext, FALLBACK.final_subtext)}</span></div>
  </div>
</section>
...
<section class="v2-section v2-dark" id="v2-close">
  <div class="v2-wrap v2-wrap--fat" style="text-align:center;">
    ...
    <div class="v2-btn-wrap v2-btn-wrap--center v2-reveal v2-delay-2"><a ${ctaHref} class="v2-btn v2-btn--inv v2-btn-pulse"><span>${heroCta}</span></a></div>
