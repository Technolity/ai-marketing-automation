 ...
 */
export function funnelCopyToRenderData(pageKey, { funnelCopy = {}, media = {}, colorPalette = {}, businessName, calendarPath } = {}) {
  ...
  // landing — calendarPath (slot-derived) bakes into the CTA hrefs
  const m = {}; put(m, 'logoUrl', logoUrl); put(m, 'bioPhotoUrl', bioPhotoUrl); put(m, 'videoUrl', heroVideoUrl);
  const landing = { brand, copy: mapLanding(funnelCopy.salesPage), media: m, embeds: {} };
  if (filled(calendarPath)) landing.calendarPath = calendarPath;
  return landing;
}
