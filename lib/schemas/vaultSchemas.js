...
export const offerSchema = z.object({
  signatureOffer: z.object({
    ...
    }).describe("7-Step Blueprint - names only"),
    ...
    }).describe("Tier 1 Signature Offer (Live Group)"),
...
export const salesScriptSchema = z.object({
  ...
  fullGuidedScript: z.object({
    part1_opening: z.string().min(50).max(1500).describe("Opening + Permission + Agenda"),
    part2_discovery: z.string().min(100).max(3000).describe("Discovery (Q1-Q7 with responses)"),
    part3_challenges: z.string().min(50).max(1500).describe("Challenges + Stakes"),
    part4_recap: z.string().min(50).max(1500).describe("Recap + Confirmation"),
    part5_decisionGate: z.string().min(20).max(800).describe("Decision Gate"),
    part6_transition: z.string().min(20).max(800).describe("Transition Into Pitch"),
    part7_pitch: z.string().min(50).max(3000).describe("Pitch (3-Step Plan)"),
    part8_investment: z.string().min(50).max(3000).describe("Investment + Value Stack + Close"),
    part9_nextSteps: z.string().min(50).max(1000).describe("Next Steps")
  }),
  objectionHandling: z.array(objectionSchema).min(5).max(20).describe("5-20 objection handlers")
});
...
const singleEmailSchema = z.object({
  ...
  body: z.string().min(250).max(3000).describe("250-600 word email body with {{contact.first_name}}, conversational tone")
});
