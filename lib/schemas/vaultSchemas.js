...
export const smsSchema = z.object({
  smsSequence: z.object({
    ...
    // DAYS 6-7: Value Reminders
    sms6: singleSmsSchema.describe("Day 6 - Final Value (one last value piece before closing)"),
    sms7a: singleSmsSchema.describe("Day 7 Morning - Value reminder"),
    sms7b: singleSmsSchema.describe("Day 7 Evening - Final value piece for the week"),

    // DAY 8: Closing Day #1
    sms8a: singleSmsSchema.describe("Day 8 Morning - Why a call helps (remove friction)"),
    sms8b: singleSmsSchema.describe("Day 8 Afternoon - Success story (social proof)"),
    sms8c: singleSmsSchema.describe("Day 8 Evening - Last chance this week (mild urgency)"),

    // DAYS 9-14: Advanced Daily Value
    sms9: singleSmsSchema.describe("Day 9 - Advanced Tip: Mindset/Strategy"),
    sms10: singleSmsSchema.describe("Day 10 - Advanced Tip: Common Mistakes"),
    sms11: singleSmsSchema.describe("Day 11 - Advanced Tip: What's Really Holding You Back"),
    sms12: singleSmsSchema.describe("Day 12 - Advanced Tip: Behind-the-Scenes Teaching"),
    sms13: singleSmsSchema.describe("Day 13 - Advanced Tip: Results Timeline"),
    sms14: singleSmsSchema.describe("Day 14 - Advanced Tip: Simplify Execution"),

    // DAY 15: Final Closing Day
    sms15a: singleSmsSchema.describe("Day 15 Morning - Final day to book consultation"),
    sms15b: singleSmsSchema.describe("Day 15 Afternoon - FAQ/Objection handling"),
    sms15c: singleSmsSchema.describe("Day 15 Evening - Final emotional push + strongest CTA"),
...
export const funnelCopySchema = z.object({
  ...
  salesPage: z.object({
    ...
    bio_paragraph_text: z.string().min(50).max(8000).describe("Bio paragraph"),
...
const reminderEmailSchema = z.object({
  ...
  body: z.string().min(100).max(3000).describe("Complete email body")
});

const reminderSmsSchema = z.object({
  timing: z.string().min(5).max(100).describe("When to send (e.g., 'Immediately upon booking', '48 hours before')"),
  message: z.string().min(10).max(300).describe("SMS message under 300 characters with {{contact.first_name}}")
});
...
export const appointmentRemindersSchema = z.object({
  appointmentReminders: z.object({
    ...
    smsReminders: z.object({
      confirmationSms: reminderSmsSchema.describe("SMS sent immediately upon booking"),
      reminder48HoursSms: reminderSmsSchema.describe("SMS sent 48 hours before"),
      reminder24HoursSms: reminderSmsSchema.describe("SMS sent 24 hours before"),
      reminder1HourSms: reminderSmsSchema.describe("SMS sent 1 hour before"),
      reminder10MinSms: reminderSmsSchema.describe("SMS sent 10 minutes before"),
      startingNowSms: reminderSmsSchema.describe("SMS sent at appointment time"),
    }).describe("SMS appointment reminders")
  })
});
...
export const colorsSchema = z.object({
  colorPalette: z.object({
    ...
    reasoning: z.string().min(50).max(1500).describe("AI reasoning for color selection and hierarchy")
  })
});
...
export const vslShortSchema = z.object({
  patternInterruptHook: z.string().min(20).max(500).describe("Bold statement that breaks a belief"),
  identifyAudience: z.string().min(20).max(500).describe("Clearly state who this video is for"),
  amplifyCorePain: z.string().min(20).max(600).describe("Describe their frustrations vividly"),
  introduceEnemy: z.string().min(20).max(500).describe("Externalize the problem"),
  revealHiddenTruth: z.string().min(20).max(500).describe("The epiphany moment"),
  earlyCTA: z.string().min(10).max(300).describe("Capture fast decision-makers"),
  authorityStory: z.string().min(50).max(800).describe("Why listen to you? Build trust"),
  namedFramework: z.string().min(30).max(500).describe("Introduce branded framework"),
  insightOne: z.string().min(50).max(600).describe("First key insight"),
  insightTwo: z.string().min(50).max(600).describe("Second key insight"),
  insightThree: z.string().min(50).max(600).describe("Third key insight"),
  microCommitment: z.string().min(20).max(300).describe("Ask viewer to mentally agree"),
  futurePace: z.string().min(30).max(500).describe("Paint the desired future"),
  objectionHandling: z.string().min(50).max(800).describe("Address common doubts"),
  riskReversal: z.string().min(30).max(500).describe("Reduce fear + explain call"),
  finalCTA: z.string().min(20).max(400).describe("Clear closing call to action")
});
