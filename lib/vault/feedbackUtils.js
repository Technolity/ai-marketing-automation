 ...
 * - optinPage: OBJECT with 5 fields
 * - salesPage: OBJECT with 69 fields
 * - calendarPage: OBJECT with 3 fields
 * - thankYouPage: OBJECT with 3 fields
 */
 ...
 * Field IDs (matching fieldStructures.js and vaultSchemas.js):
 *   preCallTips, confirmationEmail, reminder48Hours, reminder24Hours,
 *   reminder1Hour, reminder10Minutes, startingNow, noShowFollowUp, smsReminders
 */
function extractAppointmentRemindersFields(content) {
    ...
    // Legacy field name mappings (old field_ids → new field_ids)
    if (!result.confirmationEmail && ar?.confirmation) {
        result.confirmationEmail = ar.confirmation;
        found++;
    }
    if (!result.reminder24Hours && ar?.reminder24Hour) {
        result.reminder24Hours = ar.reminder24Hour;
        found++;
    }
