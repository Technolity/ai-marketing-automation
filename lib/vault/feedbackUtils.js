...
function extractIdealClientFields(content) {
    ...
    console.log('[feedbackUtils]   top3Desires:', result.top3Desires.length > 0 ? `✅ ARRAY[${result.top3Desires.length}] items` : '❌ EMPTY ARRAY');
    console.log('[feedbackUtils]   topObjections:', result.topObjections.length > 0 ? `✅ ARRAY[${result.topObjections.length}] items` : '❌ EMPTY ARRAY');
    console.log('[feedbackUtils]   topTriggers:', result.topTriggers.length > 0 ? `✅ ARRAY[${result.topTriggers.length}] items` : '❌ EMPTY ARRAY');
    console.log('[feedbackUtils]   howToTalkToThem:', result.howToTalkToThem ? `✅ "${result.howToTalkToThem.substring(0, 50)}..."` : '❌ EMPTY');
 ...
 * - bookingPage: OBJECT (mappings coming soon)
...
function extractFunnelCopyFields(content) {
    ...
    // Handle single page updates (e.g., just optinPage, salesPage, bookingPage, or thankYouPage)
    const contentKeys = Object.keys(fc || {});
    const validPages = ['optinPage', 'salesPage', 'bookingPage', 'thankYouPage'];
