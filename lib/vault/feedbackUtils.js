...
function extractIdealClientFields(content) {
    ...
    const result = {
        ...
        howToTalkToThem: Array.isArray(ic?.howToTalkToThem) ? ic.howToTalkToThem : (ic?.wordsTheyUse ? [ic.wordsTheyUse] : [])
    };
    ...
    console.log('[feedbackUtils]   howToTalkToThem:', result.howToTalkToThem.length > 0 ? `✅ ARRAY[${result.howToTalkToThem.length}] items` : '❌ EMPTY ARRAY');
