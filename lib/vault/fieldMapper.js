...
Great, I'm excited to work with you. Talk soon!`
    ...
    appointmentReminders: {
        extractFields: (content) => {
            ...
            // Helper to find email by name or timing keyword
            const findEmail = (keyword) => {
                if (!ar?.emails || !Array.isArray(ar.emails)) return {};
                return ar.emails.find(e =>
                    (e.name && e.name.toLowerCase().includes(keyword.toLowerCase())) ||
                    (e.timing && e.timing.toLowerCase().includes(keyword.toLowerCase()))
                ) || {};
            };
            ...
            const result = {
                ...
                // Map emails from array to specific fields
                confirmation: {
                    subject: findEmail('confirmation').subject || '',
                    body: findEmail('confirmation').body || ''
                },
                reminder24Hour: {
                    subject: findEmail('24-hour').subject || '',
                    body: findEmail('24-hour').body || ''
                },
                reminder1Hour: {
                    subject: findEmail('1-hour').subject || '',
                    body: findEmail('1-hour').body || ''
                },
                startingNow: {
                    subject: findEmail('starting').subject || '',
                    body: findEmail('starting').body || ''
                },
                noShowFollowup: {
                    subject: findEmail('no-show').subject || '',
                    body: findEmail('no-show').body || ''
                },

                // SMS Reminders (mapped to single object field)
                smsReminders: {
                    reminder1Day: sms.reminder1Day || '',
                    reminder1Hour: sms.reminder1Hour || '',
                    reminderNow: sms.reminderNow || ''
                }
            };

            console.log('[FieldMapper] appointmentReminders: Extracted fields:', {
                tipsCount: result.preCallTips.length,
                confirmationSubject: !!result.confirmation.subject,
                reminder24HourSubject: !!result.reminder24Hour.subject,
                smsReminder1Day: !!result.smsReminders.reminder1Day
            });
