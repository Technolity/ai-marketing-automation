...
Great, I'm excited to work with you. Talk soon!`
    ...
    funnelCopy: {
        extractFields: (content) => {
            ...
            // Flatten the structure: optinPage.headline_text -> extractedFields.headline_text
            // This allows us to map nested AI output to flat field IDs
            const result = {};

            // 1. Process Optin Page Fields
            if (fc.optinPage) {
                for (const [key, value] of Object.entries(fc.optinPage)) {
                    result[key] = value;
                }
            }

            // 2. Process Sales Page Fields
            if (fc.salesPage) {
                for (const [key, value] of Object.entries(fc.salesPage)) {
                    // Handle collision if any (though currently keys are unique across pages)
                    result[key] = value;
                }
            }

            // 3. Process Booking & Thank You (placeholders or TBD)
            if (fc.bookingPage) {
                for (const [key, value] of Object.entries(fc.bookingPage)) {
                    result[key] = value;
                }
            }

            if (fc.thankYouPage) {
                for (const [key, value] of Object.entries(fc.thankYouPage)) {
                    result[key] = value;
                }
            }

            console.log('[FieldMapper] funnelCopy: Flattened structure keys:', Object.keys(result));
            return result;
        }
    },
    colors: {
