...
const FIELD_MAPPINGS = {
    idealClient: {
        // AI output: { idealClientSnapshot: { bestIdealClient, topChallenges, whatTheyWant, whatMakesThemPay, howToTalkToThem } }
    ...
    offer: {
        extractFields: (content) => {
            ...
            const result = {
                ...
                sevenStepBlueprint: (() => {
                    const bp = offer?.sevenStepBlueprint;
                    if (Array.isArray(bp)) return bp;
                    // Convert old object format {step1: "...", step2: "..."} to array of objects
                    if (bp && typeof bp === 'object') {
                        ...
                        for (let i = 1; i <= 7; i++) {
                            ...
                            if (val) {
                                steps.push({
                                    stepName: typeof val === 'string' ? val : (val?.stepName || `Step ${i}`),
                                    whatItIs: val?.whatItIs || '',
                                    problemSolved: val?.problemSolved || '',
                                    outcomeCreated: val?.outcomeCreated || ''
                                });
                            }
                        }
                        if (steps.length > 0) {
                            console.log('[FieldMapper] Converted old sevenStepBlueprint object to array:', steps.length, 'steps');
                            return steps;
                        }
                    }
                    return [];
...
Great, I'm excited to work with you. Talk soon!`
    ...
    leadMagnet: {
        extractFields: (content) => {
            ...
            // Keep coreDeliverables as array of objects {title, description}
            const deliverables = [];
            if (Array.isArray(lm?.coreDeliverables)) {
                lm.coreDeliverables.forEach((item) => {
                    if (typeof item === 'object' && item !== null) {
                        ...
                        // Legacy: convert string to object
    ...
    bio: {
        extractFields: (content) => {
            ...
            return {
                ...
                keyAchievements: Array.isArray(bio?.keyAchievements)
                    ? bio.keyAchievements
                    : (typeof bio?.keyAchievements === 'string'
                        ? bio.keyAchievements.split('\n').map(l => l.replace(/^[\s•\-\*]+/, '').trim()).filter(l => l)
                        : [])
            };
        }
    },
    ...
    sms: {
        extractFields: (content) => {
            ...
            // Extract all 23 individual SMS messages
            const result = {
                ...
                sms8a: extractSms(sm?.sms8a),
                sms8b: extractSms(sm?.sms8b),
                sms8c: extractSms(sm?.sms8c),
                sms9: extractSms(sm?.sms9),
                sms10: extractSms(sm?.sms10),
                sms11: extractSms(sm?.sms11),
                sms12: extractSms(sm?.sms12),
                sms13: extractSms(sm?.sms13),
                sms14: extractSms(sm?.sms14),
                sms15a: extractSms(sm?.sms15a),
                sms15b: extractSms(sm?.sms15b),
                sms15c: extractSms(sm?.sms15c),
    ...
    funnelCopy: {
        extractFields: (content) => {
            ...
            console.log('[FieldMapper][FunnelCopy] fc.calendarPage exists:', !!fc.calendarPage);
            console.log('[FieldMapper][FunnelCopy] fc.thankYouPage exists:', !!fc.thankYouPage);
            ...
            const result = {
                ...
                calendarPage: fc.calendarPage || fc.bookingPage || {}, // Support legacy bookingPage name
            ...
            console.log('[FieldMapper][FunnelCopy]   - calendarPage fields:', Object.keys(result.calendarPage).length);
    ...
    appointmentReminders: {
        extractFields: (content) => {
            ...
            const ar = content?.appointmentReminders || content?.reminders || content;

            // Map contentTips object to array of strings for preCallTips field
            const tips = [];
            if (ar?.contentTips) {
                // If object (tip1, tip2...)
                if (!Array.isArray(ar.contentTips)) {
                    ['tip1', 'tip2', 'tip3'].forEach(key => {
                        const tip = ar.contentTips[key];
                        if (tip && tip.title) {
                    ...
                    // If array already
                    ar.contentTips.forEach(tip => {
                        if (typeof tip === 'string') tips.push(tip);
                        else if (tip.title) tips.push(`${tip.title}: ${tip.briefRecap || ''}`);
                    });
                }
            }

            // Helper to extract email object with subject, previewText, body
            const extractEmail = (emailObj) => {
                if (!emailObj) return { subject: '', previewText: '', body: '' };
                return {
                    subject: emailObj.subject || '',
                    previewText: emailObj.previewText || emailObj.preview || '',
                    body: emailObj.body || ''
                };
            };

            // Helper to extract SMS object (handles both string and object formats)
            const extractSms = (smsObj) => {
                if (!smsObj) return { timing: '', message: '' };
                if (typeof smsObj === 'string') return { timing: '', message: smsObj };
                return {
                    timing: smsObj.timing || '',
                    message: smsObj.message || smsObj.body || ''
                };
            };

            // Extract SMS content from smsReminders sub-object
            const sms = ar?.smsReminders || {};

            const result = {
                preCallTips: tips,

                // Named email keys matching vaultSchemas.js
                confirmationEmail: extractEmail(ar?.confirmationEmail || ar?.confirmation),
                reminder48Hours: extractEmail(ar?.reminder48Hours),
                reminder24Hours: extractEmail(ar?.reminder24Hours || ar?.reminder24Hour),
                reminder1Hour: extractEmail(ar?.reminder1Hour),
                reminder10Minutes: extractEmail(ar?.reminder10Minutes),
                startingNow: extractEmail(ar?.startingNow),
                noShowFollowUp: extractEmail(ar?.noShowFollowUp || ar?.noShowFollowup),

                // SMS Reminders (matching vaultSchemas.js smsReminders sub-object)
                smsReminders: {
                    confirmationSms: extractSms(sms.confirmationSms),
                    reminder48HoursSms: extractSms(sms.reminder48HoursSms),
                    reminder24HoursSms: extractSms(sms.reminder24HoursSms),
                    reminder1HourSms: extractSms(sms.reminder1HourSms),
                    reminder10MinSms: extractSms(sms.reminder10MinSms),
                    startingNowSms: extractSms(sms.startingNowSms)
                }
            };

            console.log('[FieldMapper] appointmentReminders: Extracted fields:', {
                tipsCount: result.preCallTips.length,
                confirmationEmailSubject: !!result.confirmationEmail.subject,
                reminder48HoursSubject: !!result.reminder48Hours.subject,
                reminder24HoursSubject: !!result.reminder24Hours.subject,
                smsConfirmation: !!result.smsReminders.confirmationSms.message
            });
    ...
    vslShort: {
        extractFields: (content) => {
            console.log('[FieldMapper] vslShort: Raw content keys:', Object.keys(content || {}));
            const vsl = content?.vslShort || content?.shortVsl || content;
            
            const result = {
                patternInterruptHook: vsl?.patternInterruptHook || '',
                identifyAudience: vsl?.identifyAudience || '',
                amplifyCorePain: vsl?.amplifyCorePain || '',
                introduceEnemy: vsl?.introduceEnemy || '',
                revealHiddenTruth: vsl?.revealHiddenTruth || '',
                earlyCTA: vsl?.earlyCTA || '',
                authorityStory: vsl?.authorityStory || '',
                namedFramework: vsl?.namedFramework || '',
                insightOne: vsl?.insightOne || '',
                insightTwo: vsl?.insightTwo || '',
                insightThree: vsl?.insightThree || '',
                microCommitment: vsl?.microCommitment || '',
                futurePace: vsl?.futurePace || '',
                objectionHandling: vsl?.objectionHandling || '',
                riskReversal: vsl?.riskReversal || '',
                finalCTA: vsl?.finalCTA || ''
            };
            
            console.log('[FieldMapper] vslShort: Extracted fields count:', Object.keys(result).filter(k => !!result[k]).length);
 ...
 * @param options - { forceOverwrite: boolean } - When true, overwrite manually-edited fields
 */
export async function populateVaultFields(funnelId, sectionId, content, userId, options = {}) {
    const { forceOverwrite = false, sourceStatus = null } = options;
    console.log(`[FieldMapper] ========================================`);
    console.log(`[FieldMapper] Populating fields for ${sectionId}, funnel: ${funnelId}${forceOverwrite ? ' [FORCE OVERWRITE]' : ''}`);
    console.log(`[FieldMapper] Content keys:`, Object.keys(content || {}));

    const mapping = FIELD_MAPPINGS[sectionId];
    if (!mapping) {
        console.error(`[FieldMapper] ✗ No mapping found for section: ${sectionId}`);
        return { success: false, error: `No field mapping for ${sectionId}` };
    }

    const fieldStructure = VAULT_FIELD_STRUCTURES[sectionId];
    if (!fieldStructure) {
        console.error(`[FieldMapper] ✗ No field structure for section: ${sectionId}`);
        return { success: false, error: `No field structure for ${sectionId}` };
    }

    try {
        // Extract fields from AI content
        console.log(`[FieldMapper] Calling extractFields for ${sectionId}...`);
        const extractedFields = mapping.extractFields(content);
        console.log(`[FieldMapper] ✓ Extracted fields:`, Object.keys(extractedFields));
        console.log(`[FieldMapper] Extracted values:`, extractedFields);
        ...
        if (existingFields && existingFields.length > 0 && !forceOverwrite) {
        ...
        } else if (forceOverwrite && existingFields && existingFields.length > 0) {
            console.log(`[FieldMapper] ⚡ Force overwrite enabled — skipping preservation of ${existingFields.length} existing fields`);
        }
        ...
        // STEP 1: Archive ALL old current fields (set is_current_version = false)
        // This prevents multiple records with is_current_version = true
        console.log(`[FieldMapper] Archiving all old current fields for ${sectionId}...`);
        const { error: archiveError } = await supabaseAdmin
            .from('vault_content_fields')
            .update({ is_current_version: false })
            .eq('funnel_id', funnelId)
            .eq('section_id', sectionId)
            .eq('is_current_version', true); // Archive ALL current fields

        if (archiveError) {
            console.error(`[FieldMapper] ✗ Archive error for ${sectionId}:`, archiveError);
        } else {
            console.log(`[FieldMapper] ✓ Archived all old current fields for ${sectionId}`);
        }

        // STEP 2: Restore manually-edited fields back to current (they were archived in STEP 1)
        if (fieldsToPreserve.length > 0) {
            console.log(`[FieldMapper] Restoring ${fieldsToPreserve.length} manually-edited fields to current...`);
            const { error: restoreError } = await supabaseAdmin
                .from('vault_content_fields')
                .update({ is_current_version: true })
                .eq('funnel_id', funnelId)
                .eq('section_id', sectionId)
                .in('field_id', fieldsToPreserve)
                .eq('is_current_version', false); // Only restore if they were just archived

            if (restoreError) {
                console.error(`[FieldMapper] ✗ Restore error for ${sectionId}:`, restoreError);
            } else {
                console.log(`[FieldMapper] ✓ Restored ${fieldsToPreserve.length} manually-edited fields`);
            }
        }

        // STEP 3: Delete old version 1 fields (cleanup)
        console.log(`[FieldMapper] Deleting archived version 1 fields for ${sectionId}...`);
        const { error: deleteError } = await supabaseAdmin
            ...
            .eq('section_id', sectionId)
            .eq('version', 1)
            .eq('is_current_version', false); // Only delete archived version 1

        if (deleteError) {
            console.error(`[FieldMapper] ✗ Delete error for ${sectionId}:`, deleteError);
        } else {
            console.log(`[FieldMapper] ✓ Cleaned up old version 1 fields for ${sectionId}`);
        }
        ...
        for (const fieldDef of fieldStructure.fields) {
            ...
            if (value !== undefined && value !== null) {
                fieldRecords.push({
                    ...
                    version: 1,
                    is_approved: sourceStatus === 'approved',
                    is_custom: false,
        ...
        if (fieldRecords.length > 0) {
            console.log(`[FieldMapper] Preparing to insert ${fieldRecords.length} field records for ${sectionId}...`);
            console.log(`[FieldMapper] Field IDs to insert:`, fieldRecords.map(f => f.field_id));
            ...
            if (error) {
                console.error(`[FieldMapper] ✗ Insert error for ${sectionId}:`, error);
                console.error(`[FieldMapper] Failed records:`, JSON.stringify(fieldRecords, null, 2));
                return { success: false, error: error.message };
            }

            console.log(`[FieldMapper] ✓ Successfully inserted ${fieldRecords.length} new fields for ${sectionId}`);
...
const fieldMapper = { populateVaultFields, populateAllVaultFields };
