...
Great, I'm excited to work with you. Talk soon!`
    ...
    facebookAds: {
        extractFields: (content) => {
            ...
            // Fallback: Old nested object structure (shortAd1, shortAd2, longAd objects)
            console.log('[FieldMapper] facebookAds: Using legacy nested object structure');
            ...
            console.log('[FieldMapper] facebookAds: Converted from nested objects to flat fields');
            return result;
        }
    },
    ...
    funnelCopy: {
        extractFields: (content) => {
            ...
            // NEW STRUCTURE: 4 pages (optinPage, salesPage, bookingPage, thankYouPage)
            // Each page is stored as an object with its subfields

            // Check if using new 4-page structure
            if (fc?.optinPage || fc?.salesPage || fc?.bookingPage || fc?.thankYouPage) {
                console.log('[FieldMapper] funnelCopy: Using NEW 4-page structure');
                ...
                console.log('[FieldMapper] funnelCopy: Extracted 4 pages:', {
                    optinPage: Object.keys(result.optinPage).length,
                    salesPage: Object.keys(result.salesPage).length,
                    bookingPage: Object.keys(result.bookingPage).length,
                    thankYouPage: Object.keys(result.thankYouPage).length
                });

                return result;
            }

            // LEGACY STRUCTURE: Convert old format to new format
            console.log('[FieldMapper] funnelCopy: Converting LEGACY structure to 4-page format');

            // Extract from legacy optInPageCopy
            const legacyOptIn = fc?.optInPageCopy || {};
            const optinPage = {
                headline_text: legacyOptIn.headline || '',
                subheadline_text: legacyOptIn.subheadline || '',
                cta_text: legacyOptIn.ctaButton || legacyOptIn.ctaButtonText || 'Get Instant Access',
                footer_company_name: ''
            };

            // Extract from legacy salesPageCopy (if object)
            const legacySales = fc?.salesPageCopy || {};
            const salesPage = typeof legacySales === 'object' ? {
                hero_headline_text: legacySales.heroHeadline || '',
                cta_text: legacySales.finalCTA || 'Book Your Free Call',
                process_headline_text: 'Here\'s How It Works',
                bio_paragraph_text: ''
            } : {};

            // Booking page (minimal in legacy)
            const bookingPage = {
                booking_pill_text: fc?.bookingPillText || 'You\'re one step away from transformation'
            };

            // Extract from legacy thankYouPageCopy
            const legacyThankYou = fc?.thankYouPageCopy || {};
            const thankYouPage = {
                headline_text: legacyThankYou.headline || 'You\'re In!',
                subheadline_text: legacyThankYou.subheadline || 'Check your email for next steps',
                testimonials_headline_text: 'What Others Are Saying',
                testimonials_subheadline_text: 'Real results from real people'
            };

            const result = { optinPage, salesPage, bookingPage, thankYouPage };

            console.log('[FieldMapper] funnelCopy: Converted legacy to 4-page format');
            return result;
        }
    },
    bio: {
    ...
    appointmentReminders: {
        extractFields: (content) => {
            ...
            // Helper to extract email object
            const extractEmail = (emailData) => {
            ...
            // Find specific emails from array by name
            const findEmail = (name) => {
                if (Array.isArray(ar?.emails)) {
                    const email = ar.emails.find(e => e.name === name);
                    return extractEmail(email);
                }
                return { subject: '', body: '' };
            };

            // Extract SMS reminders object
            const extractSMS = (sms) => {
                ...
                return { reminder1Day: '', reminder1Hour: '', reminderNow: '' };
            };

            const result = {
                preCallTips: tips,
                confirmation: findEmail('Confirmation'),
                reminder24Hour: findEmail('24-Hour Reminder'),
                reminder1Hour: findEmail('1-Hour Reminder'),
                startingNow: findEmail('Starting Now'),
                noShowFollowup: findEmail('No-Show Follow-up'),
                smsReminders: extractSMS(ar?.smsReminders)
            };

            console.log('[FieldMapper] appointmentReminders: Extracted fields:', {
                preCallTips: result.preCallTips.length,
                confirmation: !!result.confirmation.subject,
                reminder24Hour: !!result.reminder24Hour.subject,
                reminder1Hour: !!result.reminder1Hour.subject,
                startingNow: !!result.startingNow.subject,
                noShowFollowup: !!result.noShowFollowup.subject,
                smsReminders: typeof result.smsReminders
            });
    ...
    funnelCopy: {
        // AI output: { funnelCopy: { optinPage, salesPage, bookingPage, thankYouPage } }
        extractFields: (content) => {
            console.log('[FieldMapper] Raw funnelCopy keys:', Object.keys(content.funnelCopy || {}));

            const fc = content.funnelCopy || {};

            // Return entire objects for these pages - they will be handled as nested_objects
            return {
                optinPage: fc.optinPage || {},
                salesPage: fc.salesPage || {},
                bookingPage: fc.bookingPage || {},
    ...
    colors: {
        // AI output: { primaryColor, secondaryColor, accentColor, textColorDark, textColorLight, backgroundColor, backgroundSecondary, reasoning, userInputUsed }
        extractFields: (content) => {
            ...
            return {
                colorPalette: content  // Store the entire JSON object
            };
        }
    }
};
