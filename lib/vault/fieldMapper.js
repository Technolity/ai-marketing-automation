...
Great, I'm excited to work with you. Talk soon!`
    ...
    funnelCopy: {
        extractFields: (content) => {
            ...
            // Extract optInPageCopy as object (for UI object rendering)
            const extractOptInPage = (opt) => {
                if (typeof opt === 'object' && opt !== null) {
                    return {
                        headline: opt.headline || '',
                        subheadline: opt.subheadline || '',
                        heroText: opt.heroText || opt.heroSection || '',
                        bulletPoints: Array.isArray(opt.bulletPoints) ? opt.bulletPoints : [],
                        ctaButton: opt.ctaButton || opt.ctaButtonText || 'Get Instant Access',
                        socialProof: opt.socialProof || '',
                        privacyNote: opt.privacyNote || ''
                    };
                }
                return {};
            };

            // Extract thankYouPageCopy as object
            const extractThankYouPage = (ty) => {
                if (typeof ty === 'object' && ty !== null) {
                    return {
                        headline: ty.headline || '',
                        subheadline: ty.subheadline || '',
                        message: ty.message || '',
                        nextSteps: Array.isArray(ty.nextSteps) ? ty.nextSteps : [],
                        bridgeToCall: ty.bridgeToCall || '',
                        ctaButton: ty.ctaButton || ty.ctaButtonText || 'Book Your Free Strategy Call'
                    };
                }
                return {};
            };

            // Build sales page copy text (still textarea in UI)
            let salesText = '';
            if (fc?.salesPageCopy) {
...
${sp.finalCTA || ''}`;
            ...
            const result = {
                ...
                optInPageCopy: extractOptInPage(fc?.optInPageCopy),
                thankYouPageCopy: extractThankYouPage(fc?.thankYouPageCopy),
                salesPageCopy: salesText
            };

            console.log('[FieldMapper] funnelCopy: Extracted fields:', {
                optInHeadlines: result.optInHeadlines.length,
                optInPageCopy: typeof result.optInPageCopy,
                thankYouPageCopy: typeof result.thankYouPageCopy,
