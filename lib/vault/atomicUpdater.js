...
async function updateSectionContent(section, atomicChanges, sourceSectionId) {
    ...
    try {
        ...
        const usePlaceholderReplacement = sourceSectionId === 'leadMagnet';
        ...
        for (const change of atomicChanges) {
            ...
            if (usePlaceholderReplacement) {
                const placeholderResult = replaceLeadMagnetPlaceholders(contentStr, newValue);
                if (placeholderResult.replacementsCount > 0) {
                    contentStr = placeholderResult.text;
                    totalReplacements += placeholderResult.replacementsCount;
                }
                continue;
            }
...
function replaceLeadMagnetPlaceholders(text, newValue) {
    ...
    placeholders.forEach((pattern) => {
        ...
        if (matches && matches.length > 0) {
            ...
            updatedText = updatedText.replace(pattern, newValue);
        }
    });

    return { text: updatedText, replacementsCount };
}
...
async function updateFieldContent(field, oldValue, newValue, sourceSectionId, sourceFieldId) {
    ...
    try {
        ...
        let updatedValueStr = fieldValueStr;
        let replacementsCount = matchCount;

        if (sourceSectionId === 'leadMagnet') {
            const placeholderResult = replaceLeadMagnetPlaceholders(fieldValueStr, newValue);
            updatedValueStr = placeholderResult.text;
            replacementsCount = placeholderResult.replacementsCount;
        }
        ...
        console.log(`[AtomicUpdater]   Field ${field.section_id}.${field.field_id}: ${replacementsCount} occurrences`);
