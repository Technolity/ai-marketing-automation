...
export const SECTION_DEPENDENCIES = {
    ...
    21: ['idealClient', 'message', 'story', 'vsl'], // Short VSL Script
};
...
export async function buildCoreContext(funnelId) {
    ...
    // Overlay remaining Ideal Client fields from vault_fields (atomic edits take precedence)
    const granularChallenges = await fetchFieldValue(funnelId, 'idealClient', 'top3Challenges');
    if (granularChallenges && Array.isArray(granularChallenges) && granularChallenges.length > 0) {
        context.challenges = granularChallenges;
        console.log('[DependencyResolver] ✓ Overlaid challenges from vault_fields');
    }

    const granularDesires = await fetchFieldValue(funnelId, 'idealClient', 'whatTheyWant');
    if (granularDesires && Array.isArray(granularDesires) && granularDesires.length > 0) {
        context.desires = granularDesires;
        console.log('[DependencyResolver] ✓ Overlaid desires from vault_fields');
    }

    const granularTriggers = await fetchFieldValue(funnelId, 'idealClient', 'whatMakesThemPay');
    if (granularTriggers && Array.isArray(granularTriggers) && granularTriggers.length > 0) {
        context.buyingTriggers = granularTriggers;
        console.log('[DependencyResolver] ✓ Overlaid buyingTriggers from vault_fields');
    }
    ...
    // Overlay Message fields from vault_fields (atomic edits take precedence)
    // NOTE: The field may be stored as 'oneLiner' OR 'oneLineMessage' depending on the schema version
    let granularOneLiner = await fetchFieldValue(funnelId, 'message', 'oneLiner');
    if (!granularOneLiner) {
        granularOneLiner = await fetchFieldValue(funnelId, 'message', 'oneLineMessage');
    }
    if (granularOneLiner && typeof granularOneLiner === 'string' && granularOneLiner.trim()) {
        context.oneLiner = granularOneLiner;
        console.log('[DependencyResolver] ✓ Overlaid oneLiner from vault_fields');
    }

    const granularPowerLines = await fetchFieldValue(funnelId, 'message', 'powerPositioningLines');
    if (granularPowerLines && Array.isArray(granularPowerLines) && granularPowerLines.length > 0) {
        context.powerLines = granularPowerLines;
        console.log('[DependencyResolver] ✓ Overlaid powerLines from vault_fields');
    }

    // NOTE: The field may be stored as 'topThreeOutcomes' OR 'topOutcomes'
    let granularOutcomes = await fetchFieldValue(funnelId, 'message', 'topThreeOutcomes');
    if (!granularOutcomes) {
        granularOutcomes = await fetchFieldValue(funnelId, 'message', 'topOutcomes');
    }
    if (granularOutcomes && Array.isArray(granularOutcomes) && granularOutcomes.length > 0) {
        context.topOutcomes = granularOutcomes;
        console.log('[DependencyResolver] ✓ Overlaid topOutcomes from vault_fields');
    }
    ...
    // Overlay Story fields from vault_fields (atomic edits take precedence)
    const granularStorySummary = await fetchFieldValue(funnelId, 'story', 'oneLinerStory');
    if (granularStorySummary && typeof granularStorySummary === 'string' && granularStorySummary.trim()) {
        context.storySummary = granularStorySummary;
        console.log('[DependencyResolver] ✓ Overlaid storySummary from vault_fields');
    }

    const granularBigIdea = await fetchFieldValue(funnelId, 'story', 'bigIdea');
    if (granularBigIdea && typeof granularBigIdea === 'string' && granularBigIdea.trim()) {
        context.bigIdea = granularBigIdea;
        console.log('[DependencyResolver] ✓ Overlaid bigIdea from vault_fields');
    }
    ...
    // Overlay Offer fields from vault_fields (atomic edits take precedence)
    const granularOfferName = await fetchFieldValue(funnelId, 'offer', 'offerName');
    if (granularOfferName && typeof granularOfferName === 'string' && granularOfferName.trim()) {
        context.offerName = granularOfferName;
        console.log('[DependencyResolver] ✓ Overlaid offerName from vault_fields');
    }

    const granularPricing = await fetchFieldValue(funnelId, 'offer', 'tier1Investment');
    if (granularPricing && typeof granularPricing === 'string' && granularPricing.trim()) {
        context.pricing = granularPricing;
        console.log('[DependencyResolver] ✓ Overlaid pricing from vault_fields');
    }
