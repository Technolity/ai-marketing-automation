  ...
  for (let i = 0; i < chars.length; i++) {
    ...
    if (char === '"') {
      if (!inString) {
        ...
        // Check if this is really the end by looking at what follows
        // A string should be followed by: , } ] : or whitespace
        const following = chars.slice(i + 1).find(c => c !== ' ' && c !== '\n' && c !== '\r' && c !== '\t');
        
        if (following && [',', '}', ']', ':'].includes(following)) {
          ...
          // This quote is embedded in the string, escape it
          result.push('\\');
          result.push(char);
        }
      }
