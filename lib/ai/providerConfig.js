...
export const AI_PROVIDERS = {
  OPENAI: {
    ...
    name: 'OpenAI',
    models: {
      text: 'gpt-5.2', // Latest GPT-5.2 model
      image: 'dall-e-3'
    },
  ...
  GEMINI: {
    ...
    name: 'Gemini',
    models: {
      text: 'gemini-2.5-flash-image', // Gemini 2.5 Flash
    },
    timeout: 30000,
    maxRetries: 3
  }
...
// Force reset cache every 5 minutes for long-running dev servers
function ensureFreshCache() {
  const CACHE_MAX_AGE = 5 * 60 * 1000; // 5 minutes
  if (Date.now() - clientCache.lastReset > CACHE_MAX_AGE) {
    console.log('[PROVIDER] Clearing stale client cache');
    clientCache = { openai: null, claude: null, gemini: null, lastReset: Date.now() };
  }
}
...
export function getOpenAIClient() {
  ensureFreshCache(); // Clear stale cache on long-running dev servers
  ...
  if (!clientCache.openai) {
    clientCache.openai = new OpenAI({
      ...
      timeout: 120000, // 120 second timeout (was 30s - too short for complex prompts)
      maxRetries: 2,   // Reduce retries, let app-level retry handle it
    });
    console.log('[PROVIDER] OpenAI client initialized with 120s timeout');
  }
