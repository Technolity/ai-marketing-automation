...
export async function streamWithProvider(systemPrompt, userPrompt, onToken, options = {}) {
  ...
  for (const providerKey of providers) {
    ...
    console.log(`[AI STREAM] Trying ${config.name} for streaming`);
    ...
    try {
      ...
      console.log(`[AI STREAM] ${config.name} succeeded in ${responseTime}ms`);
      ...
      console.error(`[AI STREAM] ${config.name} failed after ${responseTime}ms:`, error.message);
      lastError = error;
      // Continue to next provider
    }
  }
