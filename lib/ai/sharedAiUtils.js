...
const CIRCUIT_BREAKER_THRESHOLD = 8; // Open after 8 consecutive failures (increased for better resilience)
const CIRCUIT_BREAKER_TIMEOUT = 10000; // 10 seconds before retry (faster recovery)
...
// Error metrics tracking (persists across calls in same session)
...
/**
 * Categorize and track error for monitoring
 */
export function categorizeError(error, providerKey, sectionKey) {
  let category = ERROR_CATEGORIES.NETWORK_ERROR;

  if (error.message?.includes('timeout')) category = ERROR_CATEGORIES.TIMEOUT;
  else if (error.message?.includes('stream') || error.message?.includes('closed'))
    category = ERROR_CATEGORIES.STREAM_CLOSED;
  else if (error.message?.includes('parse') || error.message?.includes('JSON'))
    category = ERROR_CATEGORIES.PARSE_ERROR;
  else if (error.code === 429) category = ERROR_CATEGORIES.RATE_LIMIT;

  // Track metrics
  global.errorMetrics.total++;
  global.errorMetrics.byCategory[category] = (global.errorMetrics.byCategory[category] || 0) + 1;
  global.errorMetrics.byProvider[providerKey] = (global.errorMetrics.byProvider[providerKey] || 0) + 1;
  if (sectionKey) {
    global.errorMetrics.bySection[sectionKey] = (global.errorMetrics.bySection[sectionKey] || 0) + 1;
  }

  console.error(`[ERROR TRACKING] Category: ${category}, Provider: ${providerKey}, Section: ${sectionKey || 'unknown'}`);
  console.error(`[ERROR TRACKING] Total errors: ${global.errorMetrics.total}`);
  console.error(`[ERROR TRACKING] By category:`, global.errorMetrics.byCategory);
...
export function getErrorMetrics() {
  return global.errorMetrics ? { ...global.errorMetrics } : null;
}
...
export async function generateWithProvider(systemPrompt, userPrompt, options = {}) {
  ...
  for (const providerKey of providers) {
    ...
    try {
      ...
      const errorCategory = categorizeError(error, providerKey, options.sectionKey);
...
async function streamWithSingleProvider(providerKey, config, systemPrompt, userPrompt, onToken, options) {
  switch (providerKey) {
    case 'OPENAI': {
      ...
      const INACTIVITY_TIMEOUT_MS = 120000; // 2 min — 30s was too aggressive for parallel load (4 concurrent streams can cause OpenAI to throttle/pause token delivery)
      ...
      for await (const chunk of stream) {
        ...
        if (token) {
          fullText += token;
          tokenCount++;
          lastTokenTime = Date.now(); // Reset heartbeat on every token
          onToken(token);
        ...
        // Detect silent stall: tokens started flowing but stopped for 30s
        const inactivityMs = Date.now() - lastTokenTime;
        if (tokenCount > 0 && inactivityMs > INACTIVITY_TIMEOUT_MS) {
          console.error('[AI STREAM] ⚠️ INACTIVITY DETECTED — no tokens for', Math.round(inactivityMs / 1000), 's after receiving', tokenCount, 'tokens (' + fullText.length + ' chars). Breaking out of stalled stream.');
          streamStalled = true;
          break;
        }
      }

      if (streamStalled && fullText.length > 0) {
        console.warn('[AI STREAM] Returning partial content from stalled stream:', {
          tokensReceived: tokenCount,
          charactersReceived: fullText.length,
          stalledAfterMs: Date.now() - streamStartTime
        });
      }
