...
export class Logger {
  ...
  info(message, context = {}) {
    const enriched = this.correlationId ? { correlationId: this.correlationId, ...context } : context;
    log(LogLevel.INFO, message, { component: this.component, ...enriched });
  }
  ...
  error(message, error = null, context = {}) {
    const baseContext = this.correlationId ? { correlationId: this.correlationId, ...context } : context;
    const errorContext = {
  ...
  /**
   * Log a step in a workflow execution
   * @param {string} stepName - Name of the step (e.g., 'generate_section_11')
   * @param {string} status - Status of the step ('started', 'completed', 'failed', 'retrying')
   * @param {object} details - Additional details (reason, attempt, etc.)
   */
  step(stepName, status, details = {}) {
    const level = status === 'failed' ? LogLevel.ERROR : LogLevel.INFO;

    log(level, `Step: ${stepName} [${status}]`, {
      component: this.component,
  ...
  /**
   * Set correlation ID for distributed tracing context
   */
  setCorrelationId(id) {
  ...
  /**
   * Internal helper to attach correlation ID to all logs
   */
  _enrichContext(context) {
    return this.correlationId ? { correlationId: this.correlationId, ...context } : context;
  }

  // ... (Overriding previous methods to use _enrichContext would be ideal, but for now we'll just add the method)
}
