...
-- RLS Policies
CREATE POLICY "Users can manage own GHL custom values"
    ON public.ghl_custom_value_mappings FOR ALL
    USING (auth.uid() = user_id);

CREATE POLICY "Admins can view all GHL custom values"
    ON public.ghl_custom_value_mappings FOR SELECT
    USING (public.is_admin());

-- Add updated_at trigger
CREATE OR REPLACE FUNCTION update_ghl_custom_values_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = now();
    RETURN NEW;
...
CREATE TRIGGER ghl_custom_values_updated_at
    BEFORE UPDATE ON public.ghl_custom_value_mappings
    FOR EACH ROW
    EXECUTE FUNCTION update_ghl_custom_values_updated_at();
